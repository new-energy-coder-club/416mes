...
test('DOM page to real IDB to localhost handler to second client confirmation, MAT unchanged',async t=>{
 ...
 ui.scan.add('issue');for(const code of ['ITM:I'])await ui.accept(code);document.getElementById('itmConfirm').click();
 for(let k=0;k<100&&!document.getElementById('itmPending').textContent.includes('执行');k++)await new Promise(r=>setTimeout(r,5));
 const submit=[...document.getElementById('itmPending').querySelectorAll('button')].find(b=>b.textContent==='执行');assert.ok(submit);submit.click();
