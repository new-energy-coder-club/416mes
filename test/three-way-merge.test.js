...
test('三方合并【核心】5000 条冲突按「表×类型×字段」分组 → 组数很少且不退化', () => {
  const conflicts = [];
  ...
  /* 这条时间断言的目的**不是给机器打分**，而是挡住算法退化：
     分组是 O(n)，5000 条正常在 10ms 量级；写成 O(n²) 会是 2500 万次比较 → 秒级。
     原来写 <50ms 太紧 —— 整套测试并行跑、且同一命令里还在 git push 时，
     实测抖动过一次假红。放宽到 500ms 仍然抓得住退化，但不会被机器负载误伤。 */
  assert.ok(ms < 500, '分组必须远低于 O(n²) 的量级（500ms 上限），实测 ' + ms + 'ms');
