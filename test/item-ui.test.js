...
test('six operation types keep original sequences with distinct Chinese step guides',()=>{const {document:d,page}=setup();for(const [kind,count,label] of [['receive',3,'入库物品'],['issue',3,'出库物品'],['transfer',5,'目标容器'],['moveContainer',2,'目标库位'],['placeContainer',2,'待定位容器'],['verifyLegacy',3,'旧物品']]){page.scan.add(kind);page.render();assert.equal(d.getElementById('itmStep').querySelectorAll('li').length,count);assert.match(d.getElementById('itmStep').textContent,new RegExp(label));assert.equal(d.getElementById('itmConfirm').disabled,true);}});
