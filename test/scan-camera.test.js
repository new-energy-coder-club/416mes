...
/* ================= 项 1：worker 侧取帧 + 项 2：自适应降档 ================= */

/* 项 1：bitmap 路径 —— 主线程零 getImageData，bitmap transfer+close，解码回流。 */
test('项 1：bitmap 路径 —— createImageBitmap 走 worker 侧取帧，主线程无 getImageData', async () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
  const { document } = parseHTML(html);
  const video = document.getElementById('scanCamVideo');
  video.videoWidth = 1920; video.videoHeight = 1080;
  ...
  const track = { stop() {} };
  const stream = { getTracks: () => [track], getVideoTracks: () => [track] };
  /* 假 canvas：getImageData 被调用即记录（应零调用）。 */
  const create = document.createElement.bind(document);
  let mainThreadGetImageData = 0;
  document.createElement = tag => {
    const elc = create(tag);
    if (String(tag).toLowerCase() === 'canvas') {
      elc.getContext = () => ({
        drawImage: () => {},
        getImageData: (x, y, w, h) => { mainThreadGetImageData++; return { width: w, height: h, data: new Uint8ClampedArray(w * h * 4) }; }
      });
    }
    return elc;
  };
  /* 假 bitmap + createImageBitmap。 */
  let bitmapClosed = 0;
  const fakeBitmap = { close() { bitmapClosed++; }, width: 1920, height: 1080 };
  let bitmapPosted = null, bitmapTransfer = null;
  class FakeWorker {
    constructor() { this.onmessage = null; this.terminated = false; }
    postMessage(payload, transfer) {
      if (payload.bitmap) {
        bitmapPosted = payload; bitmapTransfer = transfer;
        if (transfer) for (const b of transfer) { if (b && typeof b.transfer === 'function') b.transfer(); }
        setTimeout(() => this.onmessage && this.onmessage({ data: { id: payload.id, hits: [{ text: 'LOC:BM', format: '二维码' }], luma: 42 } }), 1);
      } else {
        const buf = payload.image.data;
        if (payload.image && buf && typeof buf.transfer === 'function') buf.transfer();
        setTimeout(() => this.onmessage && this.onmessage({ data: { id: payload.id, hits: [{ text: 'LOC:IMG', format: '二维码' }] } }), 1);
      }
    }
    terminate() { this.terminated = true; }
  }
  const win = Object.assign({}, document.defaultView, {
    Worker: FakeWorker,
    createImageBitmap: async v => fakeBitmap,
  ...
  await cam.open({});
  assert.ok(await waitFor(() => bitmapPosted !== null, 3000), 'bitmap 路径应触发 postMessage');
  assert.equal(bitmapPosted.bitmap, fakeBitmap, 'payload.bitmap 应 === 假 bitmap');
  assert.equal(bitmapPosted.tw, 960, '快路径 tw = 1920×0.5 = 960');
  assert.equal(bitmapPosted.th, 540);
  assert.ok(bitmapTransfer && bitmapTransfer[0] === fakeBitmap, 'bitmap 应在 transfer 列表');
  assert.ok(await waitFor(() => !document.getElementById('scanCamConfirm').hidden, 3000), 'bitmap 路径解码应出卡');
  assert.equal(document.getElementById('scanCamValue').textContent, 'LOC:BM');
  assert.equal(mainThreadGetImageData, 0, 'bitmap 路径主线程 getImageData 必须为零（worker 侧取帧），实测 ' + mainThreadGetImageData);
  cam.close('test');
});

/* 项 1：worker 回复 luma → miss 帧 lumaAvg 更新，torch 门限可用。 */
test('项 1：worker luma 回传 —— miss 帧 lumaAvg 更新（暗环境仍弹照明）', async () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
  const { document } = parseHTML(html);
  const video = document.getElementById('scanCamVideo');
  video.videoWidth = 640; video.videoHeight = 480;
  video.play = async () => {};
  const track = { stop() {}, getCapabilities: () => ({ torch: {} }), applyConstraints: async () => {}, getSettings: () => ({}) };
  const stream = { getTracks: () => [track], getVideoTracks: () => [track] };
  const fakeBitmap = { close() {}, width: 640, height: 480 };
  class FakeWorker {
    constructor() { this.onmessage = null; this.terminated = false; }
    postMessage(payload, transfer) {
      if (transfer) for (const b of transfer) { if (b && typeof b.transfer === 'function') b.transfer(); }
      setTimeout(() => this.onmessage && this.onmessage({ data: { id: payload.id, hits: [], luma: 20 } }), 1);
    }
    terminate() { this.terminated = true; }
  }
  const win = Object.assign({}, document.defaultView, {
  ...
  await cam.open({});
  /* 等 ≥24 miss 帧（torch 门限），luma=20 <90 → 应弹照明按钮。 */
  assert.ok(await waitFor(() => {
    const b = [...document.querySelectorAll('#scanCamOverlay button')].find(x => /照明/.test(x.textContent));
    return !!b;
  }, 5000), 'luma=20 暗环境 miss≥24 帧应弹「打开照明」按钮（worker luma 驱动）');
  cam.close('test');
});

/* 项 2：12 帧慢 captureMs（中位 >25ms）→ 降档 360p，canvas 目标 640×360，perf.tier。 */
test('项 2：12 帧 captureMs>25ms → 降档 360p（frameScale 0.33 + perf.tier）', async () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
  const { document } = parseHTML(html);
  const video = document.getElementById('scanCamVideo');
  video.videoWidth = 1920; video.videoHeight = 1080;
  ...
  document.createElement = tag => {
    ...
    if (String(tag).toLowerCase() === 'canvas') {
      elc.getContext = () => ({
        drawImage: (v, sx, sy, sw, sh, dx, dy, dw, dh) => { if (dw !== undefined) destSizes.push({ dw, dh }); },
        getImageData: (x, y, w, h) => ({ width: w, height: h, data: new Uint8ClampedArray(w * h * 4) })
      });
    }
    return elc;
  };
  /* 假时间轴：每次 performance.now 调用 +30ms（captureMs 恒 30>25）。 */
  let now = 1000;
  const fakeWin = Object.assign({}, document.defaultView, { performance: { now: () => { now += 30; return now; } } });
  const cam = ScanCamera.attach({
    document, win: fakeWin,
    mediaDevices: { getUserMedia: async () => stream },
    intervalMs: 2, legacyLoop: true, voteThreshold: 1,
  ...
  await cam.open({});
  assert.ok(await waitFor(() => destSizes.some(s => s.dw === 640 && s.dh === 360), 5000),
    '降档后 canvas 目标应出现 640×360，实测 sizes=' + JSON.stringify(destSizes.slice(0, 5)));
  cam.close('test');
  const p = fakeWin.__scanPerf;
  assert.equal(p.tier, '360p', 'perf.tier 应为 360p，实测 ' + p.tier);
});

/* 项 2：快序列（中位 <25ms）不降档。 */
test('项 2：captureMs 快（中位 <25ms）保持 540p 不降档', async () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
  const { document } = parseHTML(html);
  const video = document.getElementById('scanCamVideo');
  video.videoWidth = 1920; video.videoHeight = 1080;
  ...
  let now = 1000;
  const fakeWin = Object.assign({}, document.defaultView, { performance: { now: () => { now += 5; return now; } } });
  const cam = ScanCamera.attach({
    document, win: fakeWin,
    mediaDevices: { getUserMedia: async () => stream },
    intervalMs: 2, legacyLoop: true, voteThreshold: 1,
  ...
  cam.close('test');
  const p = fakeWin.__scanPerf;
  assert.equal(p.tier, '540p', '快序列应保持 540p，实测 ' + p.tier);
  assert.ok(!destSizes.some(s => s.dw === 640), '不应出现 640×360 目标');
});

/* 项 2：显式 opts.frameScale 锁定不降档。 */
test('项 2：显式 opts.frameScale 时锁定不降档', async () => {
  const html = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
  const { document } = parseHTML(html);
  const video = document.getElementById('scanCamVideo');
  video.videoWidth = 1920; video.videoHeight = 1080;
  ...
  const cam = ScanCamera.attach({
    ...
    mediaDevices: { getUserMedia: async () => stream },
    intervalMs: 2, legacyLoop: true, voteThreshold: 1,
    frameScale: 0.5,   // 显式传入 → 锁定
    decodeAll: async () => []
  });
  await cam.open({});
  await tick(200);
  cam.close('test');
  assert.equal(fakeWin.__scanPerf.tier, '540p', '显式 frameScale 应锁定 540p，实测 ' + fakeWin.__scanPerf.tier);
});
