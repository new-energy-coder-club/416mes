...
function repositoryFixture(){const state={locations:[{code:'L',status:'active'}],containers:[{code:'C',loc:'L',status:'active',version:2}],items:[{code:'I',status:'in_stock',container:'C',version:3,lastOpId:'prior'}]},logs=[];let writes=0;const faults={};return {state,logs,faults,get writes(){return writes;},async validateSchema(){if(faults.schema)throw Error('missing-column');},async operations(id){return logs.filter(o=>o.code===id);},async allOperations(){return logs.slice();},async snapshot(){return structuredClone(state);},async prepare(o){if(faults.prepare)throw Error('log timeout');const rid='rec-'+(logs.length+1);logs.push({...structuredClone(o),recordId:rid});return rid;},async apply(after,operation){writes++;if(faults.apply)throw Error('write timeout');for(const [t,rows]of Object.entries(after))for(const r of rows){const found=state[t].find(x=>x.code===r.code);if(found){if(operation&&String(operation.kind||'').startsWith('register'))throw Error('CODE_ALREADY_REGISTERED');Object.assign(found,r);}else state[t].push(structuredClone(r));}},async readAfter(after){return Object.fromEntries(Object.entries(after).map(([t,rows])=>[t,rows.map(r=>Object.fromEntries(Object.keys(r).map(k=>[k,state[t].find(x=>x.code===r.code)[k]])))]));},async finish(id,o){if(faults.finish)throw Error('finish timeout');Object.assign(logs.find(l=>l.recordId===id),o);}};}
