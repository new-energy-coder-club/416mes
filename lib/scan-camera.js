...
(function (root, factory) {
  ...
  function defaultDecode(win) {
    return async function decode(image) {
      ...
      if (win.ItemBarcode && win.ItemBarcode.frame) {
        try {
          const text = await win.ItemBarcode.frame(image);
          if (text) return { text, format: '条形码' };
        } catch (_) { /* 本帧无一维码 */ }
      }
      return null;
    };
  ...
  function attach(opts) {
    ...
    function close(reason) {
      ...
      if (had && !wasConfirmed && opts && opts.onCancel) opts.onCancel(reason || 'closed');
    }

    function pendingReset() { misses = 0; missNotified = false; }

    function showConfirm(hit, captured) {
      awaiting = true;
      clearTimeout(timer); timer = null;   // 暂停取帧，但画面保留（用户能看到对准了什么）
      ...
      confirmMeta.textContent = metaText;
      confirmBox.hidden = false;
      btnYes.disabled = !allowed;
      btnYes.textContent = allowed ? '确定填入' : '类型不符，请重扫';
      setStatus('已识别，请确认');
      btnYes.onclick = () => {
        const opts2 = current;
        close('confirmed');
        if (opts2 && opts2.onConfirm) opts2.onConfirm(hit.text, captured, hit);
      };
      btnRetry.onclick = () => { confirmBox.hidden = true; awaiting = false; pendingReset(); setStatus('正在识别…'); loop(generation); };
      btnYes.focus();
    }
    ...
    function loop(myGen) {
      if (myGen !== generation || !stream || awaiting) return;
      timer = setTimeout(async () => {
        ...
        if (image) {
          try {
            ...
            if (myGen !== generation || !stream || awaiting) return;   // 迟到结果丢弃
            if (hit && hit.text) { misses = 0; missNotified = false; showConfirm(hit, captured); return; }
    ...
    async function open(openOpts) {
      ...
      try {
        const acquired = await devices.getUserMedia({ video: { facingMode: 'environment' } });
