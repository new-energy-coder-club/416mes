...
(function (root, factory) {
  ...
  function attach(opts) {
    ...
    /* 项 2 v2（2.96）：浏览器崩溃自愈 —— 三态阶段计数。
       attach 时读 stage=parseInt(sessionStorage['mes.scanStage']||'0')；
       open() 开始（取流/worker 构造之前）立即写 stage+1 —— 崩在 getUserMedia/
       worker 构造阶段也拦不住已写入的标记。分级：
         stage=0 → 全功能；stage=1 → 兼容模式（禁 worker/wasm）；
         stage≥2 → 不调 getUserMedia、不构造 worker，直接引导文案 + 「仍要尝试」。
       任何 clean close() → removeItem 归零；stage≥2 被拦截的那次不算 clean close
       （没真正 open 过，保持不归零防崩溃循环），「仍要尝试」按钮脱困。
       旧 key mes.scanActive 顺手清理（一次性迁移）。sessionStorage 抛错整体跳过。 */
    let scanStage = 0;
    ...
    try {
      ...
      if (ss) {
        scanGuardOk = true;
        try { ss.removeItem('mes.scanActive'); } catch (_) {}   // 旧 key 迁移清理
        scanStage = parseInt(ss.getItem('mes.scanStage') || '0', 10) || 0;
      }
    } catch (_) { scanGuardOk = false; scanStage = 0; }
    let compatMode = scanStage === 1;   // stage=1：兼容模式（禁 worker），retry 可复位
    function markScanStage() {
      /* open() 开始即调用：读旧值+1 立即写回（崩溃也拦不住）。 */
      if (!scanGuardOk) return;
      try {
        const cur = parseInt(win.sessionStorage.getItem('mes.scanStage') || '0', 10) || 0;
        win.sessionStorage.setItem('mes.scanStage', String(cur + 1));
      } catch (_) {}
    }
    function clearScanStage() {
      if (!scanGuardOk) return;
      try { win.sessionStorage.removeItem('mes.scanStage'); } catch (_) {}
    }
    ...
    function getNativeDetector() {
      ...
      if (!nativeDetector) {
        try { nativeDetector = new win.BarcodeDetector(); }
    ...
    async function decodeAllVia(frame) {
      ...
      if (canvas && nativeAvailable()) {
        const hits = await nativeDecode(canvas);
    ...
    const perf = { captureMs: 0, decodeMs: 0, missCount: 0, hitCount: 0, engineName: 'jsQR+ItemBarcode', firstHitMs: 0, nativeHit: 0, workerHit: 0 };
    ...
    function captureFrame() {
      ...
      if (pipeline === 'legacy' || (!awaiting && ++frameNo % fullFrameEvery === 0)) {
        ...
        let dw = vw, dh = vh;
        if (!nativeAvailable()) {
          /* 项 2c：无原生 → 慢帧降 1280×720 中档。 */
          const sc = Math.min(1, 1280 / vw);
          dw = Math.round(vw * sc); dh = Math.round(vh * sc);
        }
        if (fullCanvas.width !== dw) fullCanvas.width = dw;
        if (fullCanvas.height !== dh) fullCanvas.height = dh;
    ...
    async function processFrame(myGen) {
      ...
      try {
        ...
        fastBusy = true;
        try {
          if (captured0.canvas && nativeAvailable()) {
            const tN0 = (win.performance && win.performance.now) ? win.performance.now() : Date.now();
            const nhits = await nativeDecode(captured0.canvas);
    ...
    async function open(openOpts) {
      ...
      if (scanGuardOk && scanStage >= 2) {
        ...
        showErr('当前浏览器无法使用相机扫码（已连续异常退出）。请用微信或系统浏览器打开本页，或改用手动输入/扫码枪。');
