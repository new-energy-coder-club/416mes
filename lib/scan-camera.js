...
(function (root, factory) {
  ...
  function attach(opts) {
    ...
    /* 项 2（2.94）：浏览器崩溃自愈 —— attach 时检查 sessionStorage 标记：
       存在 = 上次扫码会话异常终止（页面被 WebView 杀掉重载）→ 本会话进兼容模式
       （禁用 worker/wasm，走主线程链，慢但绝不崩）；正常 close 会清标记，
       下次 open 恢复全功能。sessionStorage 不可用（隐私模式）→ 整体跳过。 */
    ...
    try {
      ...
      if (ss) {
        scanGuardOk = true;
        compatMode = ss.getItem('mes.scanActive') === '1';
        if (compatMode) ss.removeItem('mes.scanActive');   // 本会话重新建立标记
      }
    } catch (_) { scanGuardOk = false; }
    function markScanActive() {
      if (!scanGuardOk) return;
      try { win.sessionStorage.setItem('mes.scanActive', '1'); } catch (_) {}
    }
    function clearScanActive() {
      if (!scanGuardOk) return;
      try { win.sessionStorage.removeItem('mes.scanActive'); } catch (_) {}
    }

    function makeWorkerBridge() {
      if (workerBridge) return workerBridge;
      if (compatMode) return null;   // 2.94：兼容模式禁用 worker（崩溃自愈）
    ...
    function close(reason) {
      ...
      /* 项 2（2.94）：任何路径的 close 都清崩溃标记（正常退出 → 下次全功能）。 */
      clearScanActive();
    ...
    async function open(openOpts) {
      ...
      try {
        ...
        /* 项 2（2.94）：open 成功 → 立崩溃标记；兼容模式给状态提示。 */
        markScanActive();
