...
(function (root, factory) {
  ...
  function attach(opts) {
    ...
    /* 项 2：frameScale/fullFrameEvery 改为会话级可变（自适应降档可中途调整）。
       用户显式传 opts.frameScale 或 opts.frameTier('540'/'360') 时锁定不降档；
       frameTier:'auto'（默认）才启用自动降档。 */
    const frameTierOpt = opts.frameTier || 'auto';
    const tierLocked = !!(opts.frameScale > 0 && opts.frameScale <= 1) || frameTierOpt === '540' || frameTierOpt === '360';
    let frameScale = frameTierOpt === '360' ? 1 / 3
      : (opts.frameScale > 0 && opts.frameScale <= 1 ? opts.frameScale : 0.5);
    let fullFrameEvery = frameTierOpt === '360' ? 6
      : (opts.fullFrameEvery >= 1 ? Math.floor(opts.fullFrameEvery) : 4);
    ...
    function makeWorkerBridge() {
      ...
      w.onmessage = ev => {
        ...
        else p.resolve({ hits: m.hits || [], luma: (typeof m.luma === 'number' ? m.luma : null) });
      };
      w.onerror = () => {
        dead = true;
      ...
      workerBridge = {
        ...
        /* 项 1：worker 侧取帧 —— 主线程 createImageBitmap(video)（GPU 侧无回读）→
           bitmap transfer 给 worker → worker 内 OffscreenCanvas 缩放 + getImageData。
           用毕 worker 侧 close；主线程这里也兜底 close（worker 崩溃时防泄漏）。 */
        decodeBitmap(bitmap, tw, th) {
          if (dead) return Promise.reject(new Error('worker dead'));
          return new Promise((resolve, reject) => {
            const id = nextId++;
            const payload = { id, bitmap, tw, th };
            const timer = setTimeout(() => {
              pending.delete(id);
              dead = true;
            ...
            pending.set(id, { resolve, reject, timer });
            try { w.postMessage(payload, [bitmap]); }
            catch (e) {
              clearTimeout(timer); pending.delete(id);
              dead = true;
              try { bitmap.close && bitmap.close(); } catch (_) {}
        ...
        async selfTest() {
          ...
          try {
            ...
            const r = await workerBridge.decodeAll({ width: n, height: n, data: rgba });
            if (dead || !r.hits || !r.hits.length) { dead = true; return false; }
            return true;
    ...
    /* P1-4：统一 decodeAll 入口 —— worker 优先，失败回退主线程链。
       修 D：worker 路径直接 transfer 了 image.data.buffer，回退时原 image 已
       detached —— 必须重新取一帧喂主线程链；取不到按 miss 处理（返回 null）。
       项 1：返回 {hits, luma} —— luma 为 worker 侧稀疏亮度（bitmap 路径）或 null。 */
    async function decodeAllVia(image) {
      const bridge = makeWorkerBridge();
      if (bridge && bridge.alive()) {
        try {
          const r = await bridge.decodeAll(image);
          perf.engineName = 'worker(zxing/jsQR)';
          return r;
        } catch (_) { /* 永久回退主线程链（用新帧） */ }
        const fresh = captureFrame();
        if (!fresh) return null;
        const hits = await chain.decodeAll(fresh);
        return { hits, luma: null };
      }
      const hits = await chain.decodeAll(image);
      return { hits, luma: null };
    }
    const decode = defaultDecode(win, async image => (await decodeAllVia(image)).hits);
    ...
    /* P1-3：框选重扫在途标志（防连点）。 */
    let cropBusy = false;
    const perf = { captureMs: 0, decodeMs: 0, missCount: 0, hitCount: 0, engineName: 'jsQR+ItemBarcode', firstHitMs: 0, tier: '540p' };
    let openAt = 0;
    ...
    function captureFrame() {
      ...
      if (pipeline === 'legacy' || (!awaiting && ++frameNo % fullFrameEvery === 0)) {
    ...
    async function cropRetake(fx, fy) {
      ...
      try {
        ...
        const r = await decodeAllVia(image);
        ...
        if (r && r.hits && r.hits.length) {
          ...
          absorbHits(r.hits, captured, 720, 720, true);
          return true;
        }
        return false;
    ...
    /* 项 1：worker 侧取帧能力门 —— createImageBitmap + OffscreenCanvas + worker 桥活着
       才启用；任一不满足走现有主线程 canvas 取帧（captureFrame）。 */
    function bitmapCaptureCapable() {
      const bridge = makeWorkerBridge();
      return !!(bridge && bridge.alive() &&
        typeof win.createImageBitmap === 'function' && typeof win.OffscreenCanvas === 'function');
    }

    /* 项 2：自适应降档 —— 最近 12 个已处理帧 captureMs 中位数 >25ms → 降 360p。
       单向（会话内不回升），open() 重置；显式 opts.frameScale/frameTier 锁定不降。 */
    const tierWindow = [];
    let tierDropped = false;
    function trackTier(captureMs) {
      perf.tier = frameScale <= 0.34 ? '360p' : '540p';
      if (tierLocked || tierDropped) return;
      tierWindow.push(captureMs);
      if (tierWindow.length > 12) tierWindow.shift();
      if (tierWindow.length < 12) return;
      const sorted = tierWindow.slice().sort((a, b) => a - b);
    ...
    /* 单帧处理：取帧 → decodeAll → 多命中汇入/未命中状态机。
       修 B：确认卡展示期间降频 —— 距上次实际处理 <200ms 的帧直接跳过（不取帧不解码，
       约 5fps）。多码发现仍活着但主线程开销降到 1/5；awaiting=false 恢复全速。
       rVFC 与 timerLoop 都经 processFrame，入口判断一处生效。
       项 1：bitmapCaptureCapable() 时改走 createImageBitmap → worker 侧取帧解码，
       主线程零 getImageData；worker 回复附 luma 供 torch 门限。 */
    let lastProcessAt = 0;
    async function processFrame(myGen) {
      ...
      const t0 = now0;
      try {
        const useBitmap = bitmapCaptureCapable();
        let image = null, bitmap = null;
        if (useBitmap) {
          const vw = video.videoWidth, vh = video.videoHeight;
          if (!vw) return false;
          bitmap = await win.createImageBitmap(video);
          const t1 = (win.performance && win.performance.now) ? win.performance.now() : Date.now();
          perf.captureMs += t1 - t0;
          trackTier(t1 - t0);
          const slow = !awaiting && frameNo % fullFrameEvery === fullFrameEvery - 1;
          const scale = slow ? 1 : frameScale;
          const tw = Math.max(1, Math.round(vw * scale));
          const th = Math.max(1, Math.round(vh * scale));
          frameNo++;
          const bridge = makeWorkerBridge();
          const r = await bridge.decodeBitmap(bitmap, tw, th);
          bitmap = null;   // transfer 后所有权归 worker
          const t2 = (win.performance && win.performance.now) ? win.performance.now() : Date.now();
          perf.decodeMs += t2 - t1;
          ...
          if (r.hits && r.hits.length) {
            ...
            absorbHits(r.hits, captured, tw, th);
          } else {
            perf.missCount++;
            misses++;
            if (r.luma != null) lumaAvg = lumaAvg === 255 ? r.luma : lumaAvg * 0.9 + r.luma * 0.1;
            missGuide();
            maybeOfferTorch();
          }
          updateCropEntry();
        ...
        if (r && r.hits && r.hits.length) {
          misses = 0; missNotified = false; guideLevel = 0;
          absorbHits(r.hits, captured, image.width, image.height);
        } else {
          perf.missCount++;
          misses++;
          sampleLuma(image);
    ...
    async function open(openOpts) {
      ...
      perf.tier = '540p';
      /* 项 2：降档状态 open() 重置（frameScale/fullFrameEvery 回到初始档）。 */
      tierWindow.length = 0; tierDropped = false;
      frameScale = frameTierOpt === '360' ? 1 / 3
        : (opts.frameScale > 0 && opts.frameScale <= 1 ? opts.frameScale : 0.5);
      fullFrameEvery = frameTierOpt === '360' ? 6
        : (opts.fullFrameEvery >= 1 ? Math.floor(opts.fullFrameEvery) : 4);
