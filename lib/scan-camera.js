  ...
  function attach(opts) {
    ...
    async function decodeAllVia(image) {
      ...
      if (bridge && bridge.alive()) {
        ...
        return chain.decodeAll(fresh0.image);
      }
    ...
    function maybeAutoZoom(hits, imgW, imgH) {
      ...
      for (const h of hits) {
        ...
        if (camCaps && camCaps.zoom) {
          if (now - lastZoomAt < 800) continue;   // 防拉锯
          try {
        ...
        } else if (bw < imgW * 0.08) {
          /* 无 zoom 能力且极小码：crop 放大重解（与快档互斥，一次一帧）。 */
    ...
    let nativeSlowDead = false;
    function nativeSlowAvailable() {
      return !nativeSlowDead && typeof win.BarcodeDetector === 'function' &&
        !!(win.BarcodeDetector.getSupportedFormats);
    }
    function captureFrame() {
      if (!video.videoWidth) return null;
      if (opts.capture) { const image = opts.capture(video); return image ? { image, isSlow: false } : null; }
      const vw = video.videoWidth, vh = video.videoHeight;
      if (pipeline === 'legacy' || (!awaiting && ++frameNo % fullFrameEvery === 0)) {
        if (!fullCtx) return null;
        let dw = vw, dh = vh;
        if (!nativeSlowAvailable()) {
        ...
        return { image: fullCtx.getImageData(0, 0, dw, dh), isSlow: true };
      }
      if (!ctx) return null;
      ...
      return { image: ctx.getImageData(0, 0, fw, fh), isSlow: false };
    }

    /* 项 2b：慢帧独立通道解码 —— 原生可用走主线程 BarcodeDetector（不占用 worker），
       否则仍走 worker（decodeAllVia）。两通道各自单飞互不阻塞。 */
    let slowBusy = false;
    async function decodeSlowNative(image) {
    ...
    async function processFrame(myGen) {
      ...
      try {
        ...
        if (isSlow) {
          /* 慢帧独立通道：原生可用走主线程 detect（与 worker 并行），否则 worker。 */
          ...
          try {
            ...
            if (nativeSlowAvailable()) {
              try {
                hits = await decodeSlowNative(image);
                perf.engineName = 'BarcodeDetector(slow)';
              } catch (_) {
                nativeSlowDead = true;
                hits = await decodeAllVia(image);
              }
            } else {
              hits = await decodeAllVia(image);
            }
