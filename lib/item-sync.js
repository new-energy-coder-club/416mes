...
(function (root, factory) {
  ...
  function proof(table, row, logs) {
    if (table === 'locations') {
      ...
      for (const o of logs) {
        ...
        /* 2.52.1（k3 审计）：after.locations 是自由文本列解析来的，可能是任何形状——
           非数组按「不算凭据」处理（fail-closed），绝不让整个 merge 崩溃。 */
        const locs = o.after && o.after.locations;
        if (!Array.isArray(locs) || !locs.some(r => r.code === row.code && r.status === 'active')) continue;
    ...
    const candidates = logs.filter(o => {
      if (o.phase !== 'APPLIED') return false;
      if (table !== 'locations' && o.code !== row.lastOpId) return false;
      const rows = o.after && o.after[table];
      if (!Array.isArray(rows)) return false;   /* 2.52.1：畸形 after 不算凭据，也不许崩 merge */
      return rows.some(r => r.code === row.code && same(group(table, r), group(table, row)));
    });
  ...
  /* P3a（用户实测「库位已启用但界面显示未启用」）：增量自愈——
     结构性时序竞争：服务端先写实体行（T1）后写 APPLIED 日志行（T2>T1）。
     客户端若恰在 T1~T2 之间轮询到实体行，merge 因无凭据拒绝采用并挂冲突；
     T2 的凭据随后到达，但实体行此后不再变化、增量拉取永不再带它 → 冲突粘死，
     界面一直显示「未启用」，直到手动全量重拉。
     revalidate 用**本地已信任的 APPLIED 日志**重放受控字段（与 merge 的 proof 同一套核对：
     locations 需 APPLIED activateLocation 凭据；containers/items 需行 lastOpId 对应的 APPLIED
     日志且 after 组精确一致、版本更高、关系有效），绝不凭空造数——
     实体行是否再次到达不再影响正确终态。返回被治愈的实体键列表。 */
  function revalidate(state) {
    ...
    for (const table of ['locations', 'containers', 'items']) {
      for (const row of (state[table] || [])) {
        const cKey = table + ':' + row.code;
        if (table === 'locations') {
          if (row.status === 'active') continue;   // LOC v1 只有单向启用；已启用无需处理
          if (!proof(table, { code: row.code, status: 'active' }, trustedLogs)) continue;
          row.status = 'active';
          delete state.__itmConflicts[cKey];
          healed.push(cKey);
          continue;
        }
        if (!row.lastOpId) continue;
        /* 本地行的 lastOpId 还是旧操作——按它找日志永远找不到「新写入」的凭据。
           正确做法：扫描全部可信 APPLIED 日志，收集 after 中含本实体的候选凭据，
           要求 after 行自洽（lastOpId === 日志 code）且版本高于本地，取版本最高者落地。
           （APPLIED 行已由服务端 readAfter 核对过，after 组即云端权威状态。） */
        let best = null;
        for (const op of trustedLogs) {
          if (op.phase !== 'APPLIED') continue;
          const rowsAfter = op.after && op.after[table];
          if (!Array.isArray(rowsAfter)) continue;
          const match = rowsAfter.filter(r => r && r.code === row.code);
          if (match.length !== 1) continue;
          const target = U.normalize(table, match[0]);
          if (String(target.lastOpId || '') !== String(op.code || '')) continue;   // after 行必须归属本操作
          if (!(target.version > (row.version || 0))) continue;                    // 版本闸门（防旧日志倒灌）
          if (!best || target.version > best.version) best = target;
        }
        if (!best) continue;
        const target = best;
        if (same(group(table, row), group(table, target))) {
          /* 本地组已与凭据一致：仅解除残留冲突（例如此前因时序竞争挂上的 unverified） */
          if (state.__itmConflicts[cKey]) { delete state.__itmConflicts[cKey]; healed.push(cKey); }
          continue;
        }
        /* 与 merge 相同的合法性闸门：物品状态合法且关系成立 */
        const validItem = table !== 'items' || U.STATES.includes(target.status) && (target.status === 'in_stock' ? !!target.container : !target.container);
        let relationValid = true;
        if (table === 'items' && target.status === 'in_stock') {
          const containers = (state.containers || []).filter(c => c.code === target.container);
          relationValid = containers.length === 1 && (state.locations || []).filter(l => l.code === containers[0].loc).length === 1;
        }
        if (table === 'containers' && target.loc) relationValid = (state.locations || []).filter(l => l.code === target.loc).length === 1;
        if (!validItem || !relationValid) continue;
        Object.assign(row, group(table, target));
        delete state.__itmConflicts[cKey];
        healed.push(cKey);
      }
    }
