...
(function (root, factory) {
  ...
  function build(txns, opts) {
    ...
    var balances = Object.create(null), out = [], deltaSum = 0;
    for (var i = 0; i < a.length; i++) {
      ...
      if ((i + 1) % every === 0) out.push({
        ...
        coverage: { minSeq: 1, maxSeq: t.seq, complete: true }, version: 1
      });
    }
  ...
  function latestUsable(checkpoints, txns) {
    ...
    var cands = (checkpoints || [])
      .filter(c => c && c.version === 1 && c.coverage && c.coverage.complete
