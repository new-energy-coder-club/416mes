...
(function (root, factory) {
  ...
  function decode(short) {
    const s = String(short || '').trim().toUpperCase().replace(/O/g, '0').replace(/[IL]/g, '1').replace(/U/g, '');   /* 2.49.5：Crockford 易混字符映射（O→0/I,L→1），手输不再必失败；校验和保护下零碰撞 */
  ...
  function parseScanText(raw) {
    ...
    const m = s.match(/\/i\/([0-9A-Za-z]{8})\/?(?:[?#].*)?$/i);   /* 2.49.5：容忍尾斜杠 */
