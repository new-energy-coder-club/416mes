  ...
  function create(opts) {
    ...
    function markAttempt(id, err) {
      var q = list().map(function (x) {
        if (x.id !== id) return x;
        return Object.assign({}, x, { tries: (x.tries || 0) + 1, lastError: err || '' });
      });
      return save(q);
    }

    /**
     * 剔除超过重试上限的条目。返回 { kept, dropped }，由调用方决定怎么告警 ——
     * 这个函数故意不自己打日志，因为「丢弃」是要被审计的行为，不能藏在库里。
     */
    function prune(limit) {
      var cap = limit == null ? maxTries : limit;
      var kept = [], dropped = [];
      list().forEach(function (x) {
        if ((x.tries || 0) >= cap) dropped.push(x); else kept.push(x);
      });
