  ...
  function createAsync(opts) {
    ...
    async function markAttempt(id, err) {
      return store.transaction(['outbox'], async function (tx) {
        ...
        x.tries = tries; x.lastError = err || ''; x.status = tries >= maxTries ? 'needs_attention' : (x.status || 'pending');
    ...
    return { async: true, store: store, maxTries: maxTries, list: list, append: append, removeById: removeById, markAttempt: markAttempt, summary: summary };
  }
