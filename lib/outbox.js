...
(function (root, factory) {
  ...
  function createAsync(opts) {
    ...
    /** 整条覆盖写回（改 status/tries/_autoRetries 等簿记字段）。
        与 markAttempt 的区别：不递增 tries、不写 lastError。
        「瞬态错误自动重试」要把 needs_attention 改回 pending 并落盘，必须靠它 ——
        早期实现漏了这个方法，调用点 ob.put(item) 抛 TypeError 又被 catch 静默吞掉，
        自动重试整段退化成每 5 分钟空转一次的死代码。 */
    async function put(item) {
      if (!item || !item.id) throw new Error('put 需要带 id 的条目');
      return store.transaction(['outbox'], async function (tx) {
        // 以库里现有条目为基础合并，避免调用方只带部分字段就把 tries/lastError 冲掉
        var cur = await tx.get('outbox', item.id);
        await tx.put('outbox', cur ? Object.assign({}, cur, item) : Object.assign({}, item));
        return tx.getAll('outbox');
      });
    }
    ...
    return { async: true, store: store, maxTries: maxTries, list: list, append: append, removeById: removeById, removeRecord: removeRecord, markAttempt: markAttempt, put: put, summary: summary };
  }
  ...
  function create(opts) {
    ...
    /** 整条覆盖写回（改 status/tries/_autoRetries 等簿记字段），不递增 tries、不写 lastError。
        与 markAttempt 的分工：markAttempt 记一次失败；put 只改状态。
        「瞬态错误自动重试」把 needs_attention 改回 pending 时必须用它，否则调用点
        ob.put(item) 抛 TypeError 又被 catch 吞掉，重试永不着陆。 */
    function put(item) {
      if (!item || !item.id) throw new Error('put 需要带 id 的条目');
      var q = list();
      var found = false;
      ...
      if (!found) q.push(Object.assign({}, item));
      return save(q);
    }
    ...
    return {
      ...
      markAttempt: markAttempt, put: put, prune: prune, clear: clear, summary: summary
    };
  }
