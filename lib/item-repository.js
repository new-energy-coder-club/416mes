  ...
  return {
    ...
    async snapshot() {
      ...
      for (const t of ['items', 'containers', 'locations', 'itemOperations']) {
        ...
        if (state.duplicates && state.duplicates[t] && state.duplicates[t].length) {
          /* 2.83.0（批量实测发现）：操作日志的重复行里，REJECTED 行是终态死分支（被杀函数重试产物）——
             真正需要拒绝的只有「同一操作ID 存在 ≥2 条非 REJECTED 行」的真歧义
             （协调器对单 opId 的 lookup 仍有 conflict 兜底；新命令用新 opId 不受影响）。 */
          if (t === 'itemOperations') {
            const live = {};
            for (const o of (state.itemOperations || [])) {
              if (!o || o.phase === 'REJECTED') continue;
              live[o.code] = (live[o.code] || 0) + 1;
            }
            if (Object.values(live).some(n => n > 1)) throw Error('DUPLICATE_ENTITY:itemOperations');
            continue;
          }
          throw Error('DUPLICATE_ENTITY:' + t);
        }
      }
      U.migrate(state);
