  ...
  function create(opts) {
    ...
    return {
      ...
      enqueue(request, sessionId, draft) {
        ...
        /* v3.13.1（BUG-11/12 修复）：**不再**在这里往 request 里补 device。
           原做法（v3.12.0）补的是内部克隆 frozen → 「落库那份有 device、提交/回执那份没有」，
           而 :142 的 `canonical(cmdRequest) !== canonical(resRequest)` 会因此判定
           RESULT_REQUEST_MISMATCH，造成**工单执行成功但本地不记账**（BUG-10/12）。
           device 现改走 header（lib/item-client.js 发 X-416mes-Device，
           lib/item-operation.js 的 declaredDevice 读），不进 request 主体，
           对 requestHash 与回执一致性校验都无影响。 */
      ...
      async recover() {
        /* v3.13.11（发现 O）：与 enqueue 同款的前置校验。
           以前这里直接用 store.getAll(...)：store 缺失或不是 indexeddb 时，
           抛出去的是底层错误（实测 "Cannot read properties of null (reading 'getAll')"），
           用户完全看不出「本机存储不可用」。enqueue 在 :ready() 里已有
           ITM_REQUIRES_INDEXEDDB（界面有中文翻译），recover 却漏了同一道闸。 */
        ready();
