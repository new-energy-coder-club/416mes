(function(root,factory){if(typeof module==='object'&&module.exports)module.exports=factory(require('./unique-items'));else root.ItemScan=factory(root.UniqueItems);})(typeof globalThis!=='undefined'?globalThis:this,function(U){
'use strict';
...
function create({getState,id}){
 ...
 function parse(text){const raw=String(text).trim();const m=raw.match(/^(LOC|CTN|ITM)[:|](.+)$/);if(!m){const r=row();if(sequences[r.kind][r.values.length]==='ITM'&&/^WP-[A-Za-z0-9_-]+$/.test(raw)){U.unique(getState(),'items',raw);return {type:'ITM',code:raw};}throw Error('请使用LOC:/CTN:/ITM:类型前缀，裸码仅接受ITM步骤已建档唯一WP码');}return {type:m[1],code:m[2]};}
 ...
 if(p.type==='ITM'){
  ...
  if(r.kind==='receive'&&!['pending','out'].includes(entity.status))throw Error('物品不能重复入库或尚待核实');
 ...
 function request(){const r=row(),v=r.values;if(v.length!==sequences[r.kind].length)throw Error('三码尚未完成');if(!r.opId)r.opId=id();const q={schemaVersion:1,opId:r.opId,kind:r.kind,expected:{containerVersion:v[1].version}};
