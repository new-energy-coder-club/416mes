...
function create({getState,id}){
 ...
 return {add,row,token,select,reset,accept,request,lock,forget,removeRow,clearUnlocked,rebuild,startBatch,stopBatch,batchState,acceptBatchCode,setBatchQty,unlock(){row().locked=false;row().generation++;},snapshot:()=>JSON.parse(JSON.stringify(session)),restore(value){session=JSON.parse(JSON.stringify(value));/* TASK-18（BUG-16）：旧草稿可能缺 rows/存着已废弃的 kind——不设防则 render 的 labels.forEach 与 add 的 sequences 查表都会炸掉整个物品页 */session.rows=(Array.isArray(session.rows)?session.rows:[]).filter(r=>r&&sequences[r.kind]&&Array.isArray(r.values));if(!session.rows.length){session.rows=[{rowId:id(),kind:'receive',generation:0,values:[],locked:false,opId:null}];}if(typeof session.active!=='number'||session.active<0||session.active>=session.rows.length)session.active=session.rows.length-1;session.rows.forEach(r=>r.generation++);}};
}
