...
'use strict';
const sequences={receive:['LOC','CTN','ITM'],issue:['ITM'],verifyLegacy:['LOC','CTN','ITM'],transfer:['LOC','CTN','ITM','LOC','CTN'],moveContainer:['CTN','LOC'],placeContainer:['LOC','CTN']};
function create({getState,id}){
 ...
 if(p.type==='ITM'){
  ...
  else if(r.kind==='transfer'&&(entity.status!=='in_stock'||entity.container!==r.values[1].code))throw Error('物品状态或来源容器不符');
