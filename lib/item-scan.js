...
function create({getState,id}){
 ...
 return {add,row,token,select,reset,accept,request,lock,forget,removeRow,clearUnlocked,rebuild,unlock(){row().locked=false;row().generation++;},snapshot:()=>JSON.parse(JSON.stringify(session)),restore(value){session=JSON.parse(JSON.stringify(value));session.rows.forEach(r=>r.generation++);}};
}
