...
(function (root, factory) {
  ...
  function entityKeysOf(req) {
    ...
    if (Array.isArray(req.items)) for (const e of req.items) {
      ...
      if (e && e.containerCode) add('containers', e.containerCode);
    }
    /* 2.65.0 P1-3（k3 S1b 实证）：收发/换箱的互斥键只留 items+containers——
       同货架不同物品的并发曾被 LOC 键误杀（三发全拒）。LOC 不在收发中被修改，
       容器版本 expected.containerVersion 有 VERSION_CONFLICT 兜底；
       move/place/activate 容器操作保留 locations 键（它们真的改 loc 归属）。 */
    const isContainerMove = ['moveContainer', 'placeContainer', 'activateContainer', 'activateLocation'].includes(req.kind);
    if (req.source) { if (req.source.container) add('containers', req.source.container); if (isContainerMove && req.source.loc) add('locations', req.source.loc); }
    if (req.target) { if (req.target.container) add('containers', req.target.container); if (isContainerMove && req.target.loc) add('locations', req.target.loc); }
