...
(function (root, factory) {
  ...
  function censusDecision(prevKeys, nextKeys, opts) {
    ...
    // 闸门 2：阈值保护
    /* 阈值取 **min**（=「超 5% **或** 超 50 条」），不是 max。
       写成 max 是 AND 语义「同时超过两者」，对小表完全失效：
       ceil(size*5%) ≤ 50 对所有 ≤1000 行的表恒成立 → 门槛恒为 50。
       实测：4 行的表丢掉 3 条（75%）竟判 ok 自动删 —— 正是方案最担心的
       「一次半截扫描清空小表」。现场 7 张主数据表里 6 张 ≤200 行，全部受影响。 */
    const threshold = Math.min(abs, Math.ceil(prev.size * ratio));
