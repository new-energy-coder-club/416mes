  ...
  function createIndexedDbStore(opts) {
      ...
      return {
        ...
        iterate: async function (name, onChunk, opts) {
          ...
          await new Promise(function (resolve, reject) {
            ...
            req.onerror = function () { reject(req.error || new Error('游标读取失败')); };
            req.onsuccess = function () {
              var cur = req.result;
              if (!cur) return resolve();
              chunk.push(clone(cur.value));
              total++;
              if (chunk.length >= size) {
                var out = chunk; chunk = [];
                Promise.resolve(onChunk(out, total)).then(function () { cur.continue(); }, reject);
                return;
              }
              cur.continue();
            };
          });
          if (chunk.length) await onChunk(chunk, total);
          return total;
        },
        replaceAll: async function (name, values) {
          assertStore(name);
    ...
    var store = {
      ...
      async iterate(name, onChunk, opts) {
        // 游标流式读：几万条流水导出时**不把整表读进内存**
        return store.transaction([name], function (tx) { return tx.iterate(name, onChunk, opts); });
      },
      async clear(name) { return store.transaction([name], function (tx) { return tx.clear(name); }); }
    };
    return store;
