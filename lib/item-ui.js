...
function mount({document:doc,getState,getPersistence,getCommands,id,mediaDevices,getClient,refreshConflicts,scanCamera,isOnline,qrSvg}){
 ...
 function reviewPanels(){
  ...
  if(banner){banner.replaceChildren();
   if(entries.length){banner.hidden=false;
    line(banner,'⚠ 有 '+entries.length+' 个编码存在未核验的云端变更，相关作业已暂停：'+entries.map(([k])=>k).join('、'));
    line(banner,'在上方填入这些编码时会给出「现场核实并启用」入口；详情与安全重拉在「建档管理」页的关系冲突区。');
 ...
 async function guidedActivate(type,code,retry,opts){
  ...
  catch(e){statusAction('启用命令提交失败：'+errText(e),'重试提交这条命令',()=>run(()=>guidedActivate(type,code,retry,opts)));return;}
 ...
 function acceptGuided(text,token){return run(async()=>{
  ...
  try{const result=scan.accept(text,token);
   ...
   el('itmCode').value='';if(prefillItem&&scan.row().values.some(v=>v.type==='ITM'&&v.code===prefillItem))prefillItem=null;status('已填写草稿，尚未提交');try{el('itmCode').focus();}catch(_){}}
  catch(e){
   ...
   if(m&&m[1]==='CTN'&&/未启用|未核实/.test(msg)){if(entityStatus('containers',m[2])==='retired'){status('该容器已退役，不能启用或作业；如属误标请联系管理员');return;}statusAction(msg,'现场确认启用该容器并继续',()=>guidedActivate('CTN',m[2],()=>acceptGuided(text,token)));return;}
 ...
 async function pending(){const box=el('itmPending');box.replaceChildren();const commands=await getCommands();if(!commands.length)line(box,'暂无待处理命令。完成步骤并确认后，会显示在这里。');for(const c of commands){const request=c.request||{},section=card(box,kindLabel(request.kind)+' · '+(c.status==='pending'?'待提交':c.status==='unknown'?'结果待确认':stateLabel(c.status)));
  ...
  line(section,'对象：'+(autoReg?'物品建档 · '+(request.entity.category||'?')+' · 待发号（提交后分配物品码）':(request.itemCode||request.containerCode||request.locationCode||request.entity?.code||'待核实')));if(request.source)line(section,'来源：'+[request.source.loc,request.source.container].filter(Boolean).join(' → '));if(request.target)line(section,'目标：'+[request.target.loc,request.target.container].filter(Boolean).join(' → '));if(request.error)line(section,'原因：'+errText(request.error));
   ...
   line(section,c.status==='pending'?'下一步：点「提交原命令」，以确认返回结果为准。':'上次提交结果未知：先「查询原命令结果」；显示已完成/已拒绝会自动清卡，显示需人工修复时勿重发。');
 meta(section,'原命令编号：'+c.id);if(getClient){const execute=method=>run(async()=>{const result=await getClient()[method](c);status(result&&result.phase==='APPLIED'?'远端已确认且本机已保存':'结果：'+stateLabel(result&&result.phase));await pending();});if(c.status==='pending'||c.status==='needs_attention')section.appendChild(button(c.status==='needs_attention'?'查询并确认原命令':'提交原命令',()=>execute('submit')));section.appendChild(button('查询原命令结果',()=>execute('query')));
 ...
 el('itmRegister').addEventListener('click',()=>run(async()=>{if(adminBusy){rstatus('上一条建档/核实提交还在进行中，请等它出结果再点');return;}adminBusy=true;try{
  ...
  if(kind!=='registerItem'){
   ...
   rstatus('建档申请已保存：'+code+'；确认前不打印正式标签');await pending();return;
  }
  ...
  if(manualCode){
   ...
   await p.enqueue({schemaVersion:1,opId:id(),kind,entity});
   rstatus('建档申请已保存：'+manualCode+'（手动码；不规范形服务端会拒收且无短链）；请在待处理区提交，确认前不打印正式标签');await pending();return;
  }
