...
function mount({document:doc,getState,getPersistence,getCommands,id,mediaDevices,getClient,refreshConflicts,clearItmConflict,scanCamera,isOnline,qrSvg}){
 ...
 if(_batch){
   ...
   _head.innerHTML='📦 批量'+(_batch.kind==='receive'?'入库':'出库')+' · 锚点 '+anchorTxt+' · <b>本批 '+_nC+'/'+n+' 件已确认</b>';
   step2.appendChild(_head);
   /* 2.99.3（用户提案·选择清单模式）：待入库/在库物品点选加入批次——
      选几生成几行的表单，逐件扫描确认（✓），最后一键一批提交 */
   const _pool=(_batch.kind==='receive'
     ?(getState().items||[]).filter(x=>['pending','out','unknown'].includes(x.status))
     :(getState().items||[]).filter(x=>x.status==='in_stock'))
     .filter(x=>!batchRows.some(r=>{const v=r.values.find(y=>y.type==='ITM');return v&&v.code===x.code;}))
     .slice(0,12);
   if(_pool.length){
     const _ph=doc.createElement('p');_ph.className='hint';
     _ph.textContent='待'+(_batch.kind==='receive'?'入库':'出库')+'物品（点选加入，再逐件扫描确认）：';
     step2.appendChild(_ph);
     const _pl=doc.createElement('div');_pl.className='itm-batch-pool';
     _pool.forEach(x=>{
       const b=doc.createElement('button');
       b.className='btn ghost small';
       b.textContent='+ '+x.code+(x.name?' '+x.name:'');
       b.addEventListener('click',()=>run(()=>{addBatchItemFromPool(x);}));
       _pl.appendChild(b);
     });
     step2.appendChild(_pl);
   }
   batchRows.slice(-5).forEach(row=>{const it=row.values.find(v=>v.type==='ITM');if(it)line(step2,(row.confirmed===false?'◌ '+it.code+'（待扫确认）':'✓ '+it.code));});
   if(n>5)line(step2,'…共 '+n+' 件');
   ...
   sticky.querySelector('#itmBatchStickyN').textContent='已确认 '+_nC3+'/'+n+' 件';
  ...
  const _st2=doc.getElementById('itmBatchSticky');if(_st2)_st2.style.display='none';
 ...
 function acceptGuided(text,token){return run(async()=>{
  ...
  try{
   ...
   if(scan.batchState()){
     ...
     if(!_bp)throw Error('无法识别编码（批量模式只收 LOC:/CTN:/ITM: 前缀码或短链）');
     let br;
     if(_bp.type==='ITM'){
 ...
 /* 2.99.3（选择清单模式）：点选加入——生成「待扫确认」行；扫描该件物理码才变 ✓ 已确认 */
 function addBatchItemFromPool(item){
  const batch=scan.batchState();
  if(!batch)throw Error('不在批量模式');
  const prevActive=scan.snapshot().active;
  const kind=batch.kind;
  if(kind==='receive'){
   if(!batch.anchor)throw Error('请先完成锚点（库位+容器）');
   const ctn=U.unique(getState(),'containers',batch.anchor.ctn);
   const snap=scan.snapshot();
   snap.rows.push({rowId:Math.random().toString(36).slice(2),kind:'receive',generation:0,
     values:[{type:'LOC',code:batch.anchor.loc,version:0},{type:'CTN',code:batch.anchor.ctn,version:ctn.version},{type:'ITM',code:item.code,version:item.version}],
     locked:false,opId:null,confirmed:false});
   // snapshot 是深拷贝——需要走 restore 写回活会话
   scan.restore(snap);
  } else {
   const pos=U.currentPosition(getState(),item.code);
   if(batch.anchor&&pos.location.code!==batch.anchor.loc)throw Error('✗ '+item.code+' 在 '+pos.location.code+'，不在本批库位 '+batch.anchor.loc);
   if(!batch.anchor)batch.anchor={loc:pos.location.code,ctn:null};
   const snap=scan.snapshot();
   snap.rows.push({rowId:Math.random().toString(36).slice(2),kind:'issue',generation:0,
     values:[{type:'ITM',code:item.code,version:item.version}],
     locked:false,opId:null,confirmed:false});
   scan.restore(snap);
  }
  scan.select(Math.min(prevActive,scan.snapshot().rows.length-1));
  status('已加入 '+item.code+'（待扫确认）——扫描该物理物品的码即变 ✓');
 }
 async function submitBatchMode(){
  ...
  status('已合成本批 '+batchRows.length+' 件为一条「'+(kind==='receive'?'批量入库':'批量出库')+'」命令（一条提交，一次完成）；请在下方提交');
  await pending();
 }
