...
function mount({document:doc,getState,getPersistence,getCommands,id,mediaDevices,getClient,refreshConflicts,clearItmConflict,scanCamera,isOnline,qrSvg}){
 ...
 if(_batch){
   ...
   line(step2,'📦 批量'+(_batch.kind==='receive'?'入库':'出库')+' · 锚点 '+anchorTxt+' · <b>本批 '+n+' 件</b>');
   batchRows.slice(-5).forEach(row=>{const it=row.values.find(v=>v.type==='ITM');if(it)line(step2,'✓ '+it.code);});
   ...
   el('itmConfirm').disabled=n===0;
   el('itmConfirm').textContent='提交本批（'+n+' 件）';
   ...
   sticky.querySelector('#itmBatchStickyN').textContent='本批 '+n+' 件';
   const sb=sticky.querySelector('#itmBatchStickySubmit');
   sb.textContent='提交本批';sb.disabled=n===0;
 } else {
 ...
 function acceptGuided(text,token){return run(async()=>{
  ...
  try{
   ...
   if(scan.batchState()){
     const _bp=(function(t){const m=/^(LOC|CTN|ITM)[:|]?\s*(.+)$/i.exec(String(t).trim());return m?{type:m[1].toUpperCase(),code:String(m[2]).trim()}:null;})(text);
     if(!_bp)throw Error('无法识别编码（批量模式只收 LOC:/CTN:/ITM: 前缀码或短链）');
     const br=scan.acceptBatchCode(_bp);
     el('itmCode').value='';
     status(br.text||'已入批');
 ...
 async function submitBatchMode(){
  ...
  const batchRows=snapshot.rows.map((row,i)=>({row,i})).filter(x=>!x.row.locked&&x.row.kind===kind&&x.row.values.some(v=>v.type==='ITM'));
  if(!batchRows.length)throw Error('本批为空：请先连扫物品');
  const st=getState();
 ...
 function abandonBatchMode(){
  ...
  const prevActive=snapshot.active;
  snapshot.rows.forEach((row,i)=>{if(!row.locked&&row.kind===batch.kind&&row.values.some(v=>v.type==='ITM')){scan.select(i);scan.removeRow(i);}});
