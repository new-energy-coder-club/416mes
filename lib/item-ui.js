...
function mount({document:doc,getState,getPersistence,getCommands,id,mediaDevices,getClient,refreshConflicts,scanCamera,isOnline,qrSvg}){
 ...
 const kinds={receive:'入库',issue:'出库',transfer:'换箱',moveContainer:'容器移库',placeContainer:'容器定位',verifyLegacy:'旧物品核实（历史）',activateLocation:'核实启用库位',activateContainer:'核实启用容器',registerItem:'物品建档',registerLocation:'库位建档',registerContainer:'容器建档',retire:'物品退役'};
 ...
 function reviewPanels(){
  ...
  if(banner){banner.replaceChildren();
   if(entries.length){banner.hidden=false;
    ...
    line(body,'涉及：'+entries.map(([k])=>k).join('、')+'。填入这些编码时会给出「现场核实并启用」入口；详情与安全重拉在「建档管理」页。');
 ...
 meta(section,'原命令编号：'+c.id);if(getClient){const execute=method=>run(async()=>{const result=await getClient()[method](c);
  ...
  status(result&&result.phase==='APPLIED'?'远端已确认且本机已保存':'结果：'+stateLabel(result&&result.phase));await pending();});if(c.status==='pending'||c.status==='needs_attention')section.appendChild(button(c.status==='needs_attention'?'查询并确认原命令':'提交原命令',()=>execute('submit')));section.appendChild(button('查询原命令结果',()=>execute('query')));
   ...
   if(c.lastError&&/TRIAL_CONCURRENT|UNRESOLVED_OPERATION_BARRIER/.test(String(c.lastError))&&c.id){
     section.appendChild(button('另一台设备在处理：排队重试',()=>run(async()=>{
       ...
       /* 2.65.0 P0-1b：退避防双机互撞 livelock：1.5s/3s/6s ±抖动，最多 3 轮自动重试 */
       for(let att=1;att<3&&r2&&r2.phase==='REJECTED'&&/TRIAL_CONCURRENT/.test(String(r2.error||''));att++){
        const waitMs=1500*Math.pow(2,att)*(0.85+Math.random()*0.3);
        status('第 '+(att+1)+' 次排队重试（约 '+Math.round(waitMs/1000)+'s 后）…');await new Promise(r=>setTimeout(r,waitMs));
        r2=await getClient().submit(nc);
       }
