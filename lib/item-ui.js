    ...
    if(c.lastError&&/TRIAL_CONCURRENT|VERSION_CONFLICT|TRIAL_PRECONDITION_CHANGED/.test(String(c.lastError))&&c.id){
      section.appendChild(button('按最新数据重建并重新提交',()=>run(async()=>{
        ...
        status(r2&&r2.phase==='APPLIED'?'重建提交成功：远端已确认'
          :(r2&&r2.phase==='REJECTED'?'本命令作废：'+errText({message:String(r2.error||'')})+'——请重新扫码确认（先确认的生效）'
          :'重建提交结果：'+stateLabel(r2&&r2.phase)+'；请在待处理区查询原命令'));
        await pending();render();
      })));
    }
    if(c.lastError&&/UNRESOLVED_OPERATION_BARRIER/.test(String(c.lastError))){
      section.appendChild(button('收口未决命令（管理员）后重试',()=>run(async()=>{
        const st=getState();
        const unsettled=(st.itemOperations||[]).filter(o=>o&&o.code&&['PREPARED','REPAIR_REQUIRED'].includes(o.phase));
        if(!unsettled.length)throw Error('本机没有未决命令的记录——请先到「物品作业」安全重拉同步操作日志，再收口');
        status('收口中…（本机记录到 '+unsettled.length+' 条未决，最多处理 8 条）');
        let done=0,failed=0;
        for(const o of unsettled.slice(0,8)){
          try{
            const client=getClient();
            if(!client||typeof client.settle!=='function')throw Error('当前客户端不支持收口');
            const r=await client.settle(o.code);
            if(r&&['APPLIED','REJECTED'].includes(r.phase))done++;else failed++;
          }catch(_){failed++;}
        }
        if(!done)throw Error('收口未成功（'+failed+' 条失败）：可能无管理员权限或日志缺失；请联系管理员处理');
        /* 收口成功 → 屏障解除 → 自动重提原命令（同一 opId：屏障拒绝发生在 claim 阶段，未留日志、编号未烧掉） */
        status('已收口 '+done+' 条未决命令，正在重新提交本命令…');
        let r2=null;
        try{r2=await getClient().submit(c);}catch(e){status('收口成功，但重新提交失败：'+errText(e)+'；请稍后在待处理区重试');await pending();return;}
        if(r2&&r2.phase==='APPLIED'){try{scan.forget(c.id);}catch(_){}}
        status(r2&&r2.phase==='APPLIED'?'收口后重提成功：远端已确认'
          :(r2&&r2.phase==='REJECTED'?'本命令作废：'+errText({message:String(r2.error||'')})+'——请重新扫码确认'
          :'收口后重提结果：'+stateLabel(r2&&r2.phase)+'；请在待处理区查询原命令'));
        await pending();render();
      })));
    }
   }}}
