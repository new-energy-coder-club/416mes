...
function submitHint(kind){return kind==='receive'?'入库':'出库';}
 ...
 for(const c of commands){const request=c.request||{},section=card(box,kindLabel(request.kind)+' · '+(c.status==='pending'?'待提交':c.status==='unknown'?'结果待确认':stateLabel(c.status)));
  ...
  if(Array.isArray(request.items)){
   ...
   meta(section,'原命令编号：'+c.id);if(getClient){const execute=method=>run(()=>executeCommand(method,c));
 ...
 function showRegisterResult(code,name,category){
  ...
  const short=LL&&LL.fromItemCode(code),link=LL&&LL.linkFor(code);
   if(short&&link){
    ...
    /* BUG-6：短链按 ItemLink 生成值原样展示——BASE_URL 与 Crockford 短码本就全大写，
       此前的 toUpperCase() 是冗余变换且会诱导用户「改成小写」反而改错短码 */
    line(box,'短链：'+link);
    /* 二维码预览复用 index.html 注入的 qrSvg（qrcode 库）；编码值=短链原值（短码大小写不敏感，decode 先归一大写） */
    const svg=typeof qrSvg==='function'?qrSvg(link):null;
