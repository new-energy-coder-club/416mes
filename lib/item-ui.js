...
function mount({document:doc,getState,getPersistence,getCommands,id,mediaDevices,getClient,refreshConflicts,scanCamera,isOnline,qrSvg}){
 ...
 const stepLabels={receive:['目标库位','目标容器','入库物品'],issue:['出库物品（自动带出当前库位/容器）'],transfer:['来源库位','来源容器','待换箱物品','目标库位','目标容器'],moveContainer:['待移动容器（自动带出当前库位）','目标库位'],placeContainer:['目标库位','待定位容器'],verifyLegacy:['核实库位','核实容器','旧物品']};
 ...
 function reviewPanels(){
  ...
  if(banner){banner.replaceChildren();
   if(entries.length){banner.hidden=false;
    ...
    line(body,'涉及：'+entries.map(([k])=>k).join('、'));
    line(body,'在上方填入这些编码时会给出「现场核实并启用」入口；详情与安全重拉在「建档管理」页的关系冲突区。');
 ...
 async function autoRestoreDraft(){
  ...
  const candidates=drafts.filter(v=>v.rows.some(r=>!r.locked&&(r.values||[]).length>0));
  if(!candidates.length)return;
  candidates.sort((a,b)=>(b.savedAt||0)-(a.savedAt||0));
  scan.restore(candidates[0]);
  render();status('已自动恢复上次未完成的扫码行；可继续填入、重扫或确认（提交后以确认结果为准）');
 }
 function render(){
  ...
  if(r.kind==='issue'&&r.values.length===1){try{const pos=U.currentPosition(getState(),r.values[0].code);if(pos.container&&pos.location)line(step,'物品现状：在库 @ '+pos.location.code+' / '+pos.container.code+'——确认后即从该位置出库');}catch(_){}}
 ...
 meta(section,'原命令编号：'+c.id);if(getClient){const execute=method=>run(async()=>{const result=await getClient()[method](c);
  ...
  if(result&&['APPLIED','REJECTED'].includes(result.phase)&&c.id){try{scan.forget(c.id);}catch(_){}}
