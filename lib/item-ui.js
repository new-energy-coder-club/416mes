...
function mount({document:doc,getState,getPersistence,getCommands,id,mediaDevices,getClient}){
 ...
 function stopCamera(){cameraGeneration++;clearTimeout(timer);timer=null;if(stream)stream.getTracks().forEach(t=>t.stop());stream=null;el('itmVideo').srcObject=null;el('itmVideo').style.display='none';}
