...
function mount({document:doc,getState,getPersistence,getCommands,id,mediaDevices,getClient,refreshConflicts,clearItmConflict,scanCamera,isOnline,qrSvg}){
 ...
 async function mergeSubmitBatch(kind,rowIndexes){
  ...
  collected.forEach(c=>{const snap2=scan.snapshot();const r=snap2.rows[c.row];if(r){r.locked=true;r.opId=batchOpId;r.generation++;}});
 ...
 /* 2.83.0（批量 UI 补全）：同类同库位的草稿行 ≥2 时，提供「合并提交本批」——
    逐行命令仍是默认；合批把 N 行合成一条 receiveBatch/issueBatch 命令，
    同容器串行墙下从 N 条慢命令变 1 条。 */
 const _mergeables=(function(){
   const snap=scan.snapshot();
   const groups={};
 ...
 ['receive','issue'].forEach(kind=>{
   const idxs=_mergeables[kind]||[];
   if(idxs.length<2)return;
   psum.appendChild(doc.createTextNode('　'));
   psum.appendChild(button('合并提交本批（'+(kind==='receive'?'入库':'出库')+' '+idxs.length+' 件）',()=>run(async()=>{await mergeSubmitBatch(kind,idxs);})));
 });
