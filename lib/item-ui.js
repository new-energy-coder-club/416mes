...
function mount({document:doc,getState,getPersistence,getCommands,id,mediaDevices,getClient,refreshConflicts,scanCamera,isOnline,qrSvg}){
 ...
 function acceptGuided(text,token){return run(async()=>{
  ...
  try{const result=scan.accept(text,token);
   ...
   el('itmCode').value='';if(prefillItem&&scan.row().values.some(v=>v.type==='ITM'&&v.code===prefillItem))prefillItem=null;status('已填写草稿，尚未提交');}
  catch(e){
 ...
 el('itmCamera').addEventListener('click',()=>run(async()=>{
  ...
  await c.open({
   ...
   onConfirm:(text)=>{el('itmCode').value=text;const r=scan.row(),labels=stepLabels[r.kind]||[];const stepName=labels[r.values.length]||'当前步骤';status('已填入输入框，点「填入本步骤」或回车确认到当前步骤（'+stepName+'）');},
