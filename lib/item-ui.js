...
function mount({document:doc,getState,getPersistence,getCommands,id,mediaDevices,getClient,refreshConflicts,scanCamera,isOnline,qrSvg}){
 ...
 async function autoRestoreDraft(){
  ...
  const candidates=drafts.filter(v=>v.rows.every(r=>{
  ...
  })).filter(v=>v.rows.some(r=>!r.locked&&(r.values||[]).length>0));
  if(!candidates.length)return;
  draftsDropped=drafts.length-candidates.length;
 ...
 el('itmRowDelete').addEventListener('click',()=>run(()=>{const idx=scan.snapshot().active;scan.removeRow(idx);status('已删除第 '+(idx+1)+' 行草稿');}));
