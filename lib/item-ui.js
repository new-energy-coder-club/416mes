...
function mount({document:doc,getState,getPersistence,getCommands,id,mediaDevices,getClient,refreshConflicts,scanCamera}){
 ...
 function acceptGuided(text,token){return run(async()=>{
  ...
  try{scan.accept(text,token);el('itmCode').value='';status('已填写草稿，尚未提交');}
  catch(e){
   const msg=(e&&e.message)||String(e);
   const m=String(text).trim().match(/^(LOC|CTN|ITM)[:|](.+)$/);
   ...
   if(m&&m[1]==='ITM'&&(/NOT_FOUND|不存在/.test(msg)||(e&&e.code)==='NOT_FOUND')){statusAction('该物品码未建档：'+m[2],'以该码建档（先注册，确认后重新入库）',()=>{const det=el('itmRegister').closest('details');if(det)det.open=true;const t=el('itmRegisterType');for(const o of t.options)o.selected=o.value==='registerItem';el('itmRegisterCode').value=m[2];status('已把 '+m[2]+' 带入建档区，确认后请重新按步骤扫码');});return;}
 ...
 async function pending(){const box=el('itmPending');box.replaceChildren();const commands=await getCommands();if(!commands.length)line(box,'暂无待处理命令。完成步骤并确认后，会显示在这里。');for(const c of commands){const request=c.request||{},section=card(box,kindLabel(request.kind)+' · '+(c.status==='pending'?'待提交':c.status==='unknown'?'结果待确认':stateLabel(c.status)));line(section,'对象：'+(request.itemCode||request.containerCode||request.locationCode||request.entity?.code||'待核实'));if(request.source)line(section,'来源：'+[request.source.loc,request.source.container].filter(Boolean).join(' → '));if(request.target)line(section,'目标：'+[request.target.loc,request.target.container].filter(Boolean).join(' → '));if(request.error)line(section,'原因：'+errText(request.error));
 ...
 el('itmRegister').addEventListener('click',()=>run(async()=>{if(adminBusy)return;adminBusy=true;try{const kind=el('itmRegisterType').value||'registerItem',prefix={registerItem:'WP-',registerLocation:'LOC-',registerContainer:'CTN-'}[kind],manualCode=(el('itmRegisterCode')&&el('itmRegisterCode').value.trim())||'',code=manualCode||prefix+id();const name=el('itmRegisterName').value.trim();const entity={code,...(kind==='registerItem'?{name}:kind==='registerLocation'?{desc:name}:{spec:name})};const p=getPersistence();if(!p)throw Error('IDB不可用');await p.enqueue({schemaVersion:1,opId:id(),kind,entity});status('建档申请已保存：'+code+'；确认前不打印正式标签');await pending();}finally{adminBusy=false;}}));
