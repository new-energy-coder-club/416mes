...
function mount({document:doc,getState,getPersistence,getCommands,id,mediaDevices,getClient,refreshConflicts,clearItmConflict,scanCamera,isOnline,qrSvg}){
 ...
 async function guidedActivate(type,code,retry,opts){
  ...
  try{result=await getClient().submit(cmd);}
   ...
   statusAction('启用命令提交失败：'+errText(e),'安全重拉核对后查询结果',()=>run(async()=>{
    ...
    let qr;try{qr=await getClient().query(cmd);}catch(e2){status('查询失败：'+errText(e2)+'；请稍后在待处理区查询云端结果');return;}
    await pending();
  ...
  status('启用结果待确认（'+stateLabel(result&&result.phase)+'），请在待处理区查询云端结果，勿重复操作');
 }
 ...
 for(const c of commands){const request=c.request||{},section=card(box,kindLabel(request.kind)+' · '+(c.status==='pending'?'待提交':c.status==='unknown'?'结果待确认':stateLabel(c.status)));
  ...
  if(Array.isArray(request.items)){
   ...
   :'这条命令的云端结果还没回来：点「查询云端结果」查看（已完成会自动清卡）；也可以按最新数据重试，或直接删除本机记录。');
   meta(section,'原命令编号：'+c.id);if(getClient){const execute=method=>run(()=>executeCommand(method,c));if(c.status==='pending'||c.status==='needs_attention')section.appendChild(button(c.status==='needs_attention'?'重新执行':'执行',()=>execute('submit')));section.appendChild(button('查询云端结果',()=>execute('query')));
    ...
    /* v3.4.0 R3：取消/作废统一为「删除记录」——用户无法建立「取消」与「作废」两个动词的预期差 */
    if(c.status==='pending'&&_pp&&typeof _pp.abandonCommand==='function')section.appendChild(button('删除记录（不执行）',()=>run(async()=>{
      if(!confirm('删除这条「'+kindLabel(request.kind)+'」记录？\n\n删除后不会执行、数据不动，这张卡消失。\n若该对象此前已建档成功，结果保留，可在「资源档案」删除。'))return;
      await _pp.abandonCommand(c.id);try{scan.forget(c.id);}catch(_){ }
      status('已删除记录：'+kindLabel(request.kind));await pending();render();
    })));
    /* C1/C2（v3.3.1，用户实测「上次没同意的命令堆在待处理区删不掉」死锁）：
       提交过一次的卡（结果未回/需管理员处理）此前没有任何删除或收口出口。
       ① 「删除记录」（本地作废，用户拍板）：只删本机跟踪，云端数据不变；
          云端未决行由既有自动收口机制在下次操作时按实际状态判定。
       ② 「核对云端实际状态」（settle）：立即回读实体实际状态三态判定
          （一致→APPLIED/不一致→REJECTED），收口到终态后复用查询回执路径清卡/落卡。
          服务端要求管理员角色，按钮文案如实标注。 */
    if(_pp&&typeof _pp.abandonCommand==='function'&&c.status!=='pending'&&!['APPLIED','REJECTED'].includes(c.status))section.appendChild(button('删除记录（不再跟踪）',()=>run(async()=>{
      if(!confirm('删除这条「'+kindLabel(request.kind)+'」记录？\n\n删除只删本机这张卡，云端数据不变：\n· 若云端已生效，结果保留不变（建议稍后安全重拉核对）\n· 若云端仍未决，会在下次操作时自动按实际状态收口\n\n删除后本机不再跟踪此命令。'))return;
      await _pp.abandonCommand(c.id);try{scan.forget(c.id);}catch(_){ }
      status('已删除记录：'+kindLabel(request.kind)+'；本机不再跟踪此命令（建议稍后安全重拉核对云端实际状态）');await pending();render();
    })));
    if(getClient()&&typeof getClient().settle==='function'&&c.status!=='pending'&&!['APPLIED','REJECTED'].includes(c.status))section.appendChild(button('核对云端实际状态（需管理员）',()=>run(async()=>{
      const client=getClient();
      if(!confirm('核对这条「'+kindLabel(request.kind)+'」命令？\n\n系统会先回读云端实体实际状态再判定：\n· 实体与命令目标一致 → 判定「已生效」\n· 不一致 → 判定「未生效」（如实标记，数据不动）\n\n不会重复执行命令，可以放心操作。'))return;
      status('核对中…（回读实体实际状态后判定，不会重复执行命令）');
      let r=null;
      try{r=await client.settle(c.id);}
      catch(e){if(/FORBIDDEN/.test(String(e.message||e)))throw Error('无核对权限（需管理员）；可先用「删除记录（不再跟踪）」解除本机跟踪，云端命令会在下次操作时自动收口');throw e;}
      if(r&&['APPLIED','REJECTED'].includes(r.phase)){
        status('云端已核对：'+stateLabel(r.phase)+'；正在同步本机记录…');
        await executeCommand('query',c);   /* GET 同 opId → 终态回执路径（acknowledge 清卡/重建落卡） */
      }else{
        status('核对结果：'+stateLabel(r&&r.phase)+(r&&r.error?'：'+errText({message:String(r.error||'')}):'')+'；云端仍未判定，请稍后重试或联系管理员');await pending();
      }
    })));
   ...
   if(request.error&&/SOURCE_MISMATCH/.test(String(request.error))&&refreshConflicts)section.appendChild(button('安全重拉核对后重新扫码',()=>run(async()=>{await refreshConflicts();status('已重拉核对；本机数据已与云端对齐，请重新扫码出库');})));
    /* P1b/P2a（用户实测 + 并发模型定稿）：被拒命令的两条出口——
       ① 版本类拒绝（本机数据陈旧/同项先到者赢）：「按最新数据重试」只跑一轮：
     ...
     if(c.lastError&&/TRIAL_CONCURRENT|VERSION_CONFLICT|TRIAL_PRECONDITION_CHANGED/.test(String(c.lastError))&&c.id){
       section.appendChild(button('按最新数据重试',()=>run(async()=>{
        ...
        status(r2&&r2.phase==='APPLIED'?'重建提交成功：远端已确认'
          ...
          :'重建提交结果：'+stateLabel(r2&&r2.phase)+'；请在待处理区查询云端结果'));
