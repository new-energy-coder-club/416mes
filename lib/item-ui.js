...
'use strict';
function mount({document:doc,getState,getPersistence,getCommands,id,mediaDevices}){
 ...
 function render(){const snapshot=scan.snapshot(),r=scan.row();el('itmRows').replaceChildren();snapshot.rows.forEach((row,index)=>{const o=doc.createElement('option');o.value=String(index);o.textContent=(index+1)+' · '+row.kind+(row.locked?' 已锁定':'');o.selected=index===snapshot.active;el('itmRows').appendChild(o);});el('itmStep').textContent='行 '+r.rowId+'；当前步骤 '+(Scan.sequences[r.kind][r.values.length]||'待确认')+'；'+r.values.map(v=>v.type+':'+v.code).join(' → ')+'；数量固定1';el('itmConfirm').disabled=r.locked||r.values.length!==Scan.sequences[r.kind].length;}
 ...
 async function pending(){const box=el('itmPending');box.replaceChildren();for(const c of await getCommands())line(box,'待提交/待确认 '+c.id+' · '+c.status+' · '+(c.request.itemCode||c.request.containerCode||''));}
 let stream=null,timer=null,cameraGeneration=0;
 function stopCamera(){cameraGeneration++;clearTimeout(timer);timer=null;if(stream)stream.getTracks().forEach(t=>t.stop());stream=null;el('itmVideo').style.display='none';}
 ...
 el('itmCamera').addEventListener('click',()=>run(async()=>{
  ...
  const video=el('itmVideo');video.srcObject=stream;video.style.display='block';await video.play();
 ...
 render();return {scan,render,search,detail,accept,pending,stopCamera};
}
return {mount};
});
