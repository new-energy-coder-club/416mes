...
function mount({document:doc,getState,getPersistence,getCommands,id,mediaDevices,getClient,refreshConflicts,scanCamera,isOnline,qrSvg}){
 ...
 const stepLabels={receive:['目标库位','目标容器','入库物品'],issue:['出库物品（自动带出当前库位/容器）'],transfer:['待换箱物品（自动带出现状）','目标库位','目标容器'],moveContainer:['待移动容器（自动带出当前库位）','目标库位'],placeContainer:['目标库位','待定位容器'],verifyLegacy:['核实库位','核实容器','旧物品']};
