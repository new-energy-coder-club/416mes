...
/**
 * 带类型的表结构缓存（短 TTL）。
 * 写入路径每次都要 listFields（约 0.7 秒），而表结构几乎不变；
 * 缓存后热实例上直接 0ms。TTL 取 5 分钟，列有增删最多 5 分钟内自动跟上。
 */
const _fieldsTs = Object.create(null);
const FIELDS_TTL_MS = 5 * 60 * 1000;
async function listFieldsCached(token, tableId) {
  const now = Date.now();
  if (_fieldCache[tableId] && _fieldCacheFull[tableId] && now - (_fieldsTs[tableId] || 0) < FIELDS_TTL_MS) {
    return _fieldCacheFull[tableId];
  }
...
async function sumDeltas(token, matCode) {
  ...
  try {
    ...
    do {
      const d = await searchRecords(token, TABLES.transactions, {
        pageSize: 500, pageToken: pt || undefined, field_names: ['物料码', '变动'],
  ...
  return rows.reduce((s, r) => s + N(r.fields['变动']), 0);
}
 ...
 */
async function writeStock({ matCode, qty, delta, operator, type, reason, ref, opId, dryRun }) {
  ...
  /* 1) 预读并行化。
     这三件事互不依赖：找物料行、读流水表结构、（顺便）先把物料当前值拿到。
     原来是 3 次顺序调用 ≈ 2.4 秒，并行后 ≈ 0.8 秒。
     表结构走短 TTL 缓存，热实例上基本 0ms。 */
  let hit, txnDefs, dup;
  try {
    const t = Date.now();
    /* 三件事互不依赖，全部并起来：
         · 找物料行
         · 读流水表结构（短 TTL 缓存，热实例 0ms）
         · 幂等查重（操作ID 是否已落库）
       幂等查询即使「操作ID 这一列还不存在」也只是抛错被吞掉，不影响别的结果，
       所以可以放心并进同一批 —— 少一次串行往返（约 0.9 秒）。 */
    const [hitR, defsR, dupR] = await Promise.all([
      findOne(token, TABLES.materials, '物料码', matCode),
      listFieldsCached(token, TABLES.transactions),
      (opId ? findOne(token, TABLES.transactions, '操作ID', String(opId)).catch(() => null) : Promise.resolve(null))
    ]);
    hit = hitR; txnDefs = defsR; dup = dupR;
    bump(); bump(); if (opId) bump();
    mark('lookup', t);
  } catch (e) { return { ok: false, error: String((e && e.message) || e) }; }
  ...
  /* 期初 = 当前 qty − 现有账本求和。之后一切以「期初 + Σδ」为准，
     这样既保住了历史（期初吸收了账本看不到的部分），又让 qty 变成账本的函数。 */
  let sumBefore = 0;
  try { const t = Date.now(); sumBefore = await sumDeltas(token, matCode); bump(); timing.lookup += Date.now() - t; } catch (_) { sumBefore = 0; }
  const opening = before - sumBefore;
  ...
  let lastErr = null;
  let sumPrecomputed = null;      // 与撞号检测并行取回的账本求和，供 qty 收敛复用
  for (let attempt = 0; attempt < SEQ_TRIES; attempt++) {
    ...
    try {
      ...
      /* 撞号检测与「账本求和」互不依赖，合并成一次并行 —— 原来串行要 ~2.5 秒。
         求和结果顺手交给下面的 qty 收敛循环用，省掉重复的一次读。 */
      const [same, sumFirst] = await Promise.all([
        findMany(token, TABLES.transactions, '流水号', '#' + pad(seq, 6)),
        sumDeltas(token, matCode).catch(() => null)
      ]);
      bump(); bump(); timing.verify += Date.now() - tVerify;
      if (sumFirst !== null) sumPrecomputed = sumFirst;
      if (same.length <= 1) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    if (sumPrecomputed !== null) { sumNow = sumPrecomputed; sumPrecomputed = null; }
    else {
    ...
    balance = want;
    /* 账本求和恰好等于「期初 + 本次增量」时，说明这段时间内**没有别的写入者**。
       库存数量只会在写流水之后被改，所以没人能覆盖我们刚写进去的值 —— 这次回读可以省。
       反之（求和多了/少了）说明有并发写入，必须回读确认并再收敛一轮。 */
    if (Math.abs(sumNow - (sumBefore + actualDelta)) < 1e-9) { converged = true; break; }
...
async function censusTable(keyName, opts) {
  ...
  try {
    const keys = [];
    let pageToken = '', expected = null, complete = true;
    do {
      const d = await searchRecords(token, tableId, {
        pageSize,
        pageToken: pageToken || undefined,
    ...
    if (expected !== null && keys.length !== expected) complete = false;   // 收到数 != total → 不许判删
    return { keys, total: expected, complete, scanned: keys.length, source: 'search' };
  } catch (e) {
...
async function maxCodeSuffix(p) {
  ...
  let items = [];
  try {
    const filter = (p.type && typeColumn)
      ? { conjunction: 'and', conditions: [{ field_name: typeColumn, operator: 'is', value: [String(p.type)] }] }
      : undefined;
    const d = await searchRecords(token, tableId, {
      pageSize: 500,
...
module.exports = {
  ...
  batchUpdate, batchCreate, coerceFields, mapUp, mapDown, upsertRecords, deleteRecords, writeStock, listFieldsCached,
  searchRecords, probeChangeDetection, dumpSearchShape,
  LAST_UPDATED_COL, probeTableChange, probeAllChanges, pullChangesBySort, censusTable, benchFeishu, syncChanges,
