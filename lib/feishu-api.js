 ...
 * 返回 { fields, dropped, droppedColumns }：dropped 说明每个被跳过的列及原因。
 */
function coerceFields(defs, fields) {
  ...
  const drop = (k, why) => { dropped.push(why ? (k + '（' + why + '）') : k); droppedColumns[k] = why || '表里没有这一列'; };
  Object.entries(fields || {}).forEach(([k, v]) => {
    ...
    switch (d.typeName) {
      ...
      default:
        ...
        if (!empty) out[k] = v;
        else drop(k, d.typeName + '类型不接受空值，已跳过');
    }
  });
...
async function upsertRecords(keyName, records, opts) {
  ...
  (records || []).forEach(r => {
    ...
    Object.keys(c.droppedColumns).forEach(col => {
      // 业务键本身被丢掉的（表里没有主键列）不该进保护名单，那属于表结构错误
      if (col === def.key) return;
      (localsOfCol[col] || []).forEach(f => bad.add(f));
    });
...
async function reconcile(local) {
  ...
  await Promise.all(PUSH_ORDER.map(async key => {
    ...
    (def.fields || []).forEach(([localField, col]) => {
      ...
      const used = new Set();
      ((local && local[key]) || []).forEach(r => { const v = r[localField]; if (v !== '' && v != null) used.add(String(v)); });
