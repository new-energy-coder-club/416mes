...
async function writeStock({ matCode, qty, delta, operator, type, reason, ref, opId, dryRun }) {
  ...
  let txnDefs;
  try { const t = Date.now(); txnDefs = await listFieldsCached(token, TABLES.transactions); bump(); timing.lookup += Date.now() - t; }
  catch (e) {
  ...
  try {
    const t = Date.now();
    const li = await ledgerInfo(token, matCode);
    bump(); timing.lookup += Date.now() - t;
    sumBefore = li.sum; opening = li.opening;
  } catch (_) { sumBefore = 0; opening = null; }
