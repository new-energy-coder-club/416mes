...
// 用户已在本Base新增“物品操作”，Table ID通过只读浏览器核实。
// 自定义Base/测试配置不继承此ID；明确环境配置仍优先。
if (!TABLES.itemOperations && BASE_TOKEN === 'NpWBb0RXYayqfosYqnScWdlDnMb' && FEISHU_HOST === 'https://open.feishu.cn') {
  TABLES.itemOperations = process.env.FEISHU_ITEM_OPERATIONS_TABLE || 'tblWyVuqDDBnU05t';
}
...
/**
 * 物品化工单判别（与 mes-core.isItemizedOrder 同规则；本文件不依赖 mes-core）。
 * 物品化明细行是 {itemCodes:[…]}，没有 matCode/qty。
 */
function isItemizedOrderShape(o) {
  return !!o && (o.itemized === true ||
    (Array.isArray(o.items) && o.items.length > 0 && !!o.items[0] && Array.isArray(o.items[0].itemCodes)));
}
...
const UP = {
  ...
  /* 明细列双形态：物品化行 → 物品码列表（'; ' 连接）；旧行 → matCodexqty。
     同一单元格内两种 token 混排是合法的（DOWN 按 token 逐个分流）。 */
  items: v => (Array.isArray(v) && v.length ? v.map(i => {
    if (i && Array.isArray(i.itemCodes)) return i.itemCodes.join('; ');
    return i.matCode + 'x' + i.qty;
  }).filter(Boolean).join('; ') : ''),
...
const DOWN = {
  ...
  /* 明细列双形态解析：token 以「x数量」结尾 → 旧形态 {matCode, qty}；
     否则 → 物品码，集中成一行 {itemCodes:[…]}（物品化工单的计划就是码的集合）。 */
  items: v => {
    const rows = [], codes = [];
    T(v).split(';').map(s => s.trim()).filter(Boolean).forEach(s => {
  ...
  /* 执行数量列双形态解析：token 以「=数量」结尾 → 旧形态；否则 → 已 APPLIED 的物品码。
     物品码行进 mapDown 时再按工单形态拆到 execItems（见 mapDown 的 workorders 特例）。 */
  execqty: v => {
    const rows = [], codes = [];
    T(v).split(';').map(s => s.trim()).filter(Boolean).forEach(s => {
      const m = s.match(/^(.+?)=(\d+(?:\.\d+)?)$/);
...
const TABLE_DEFS = {
  ...
  containers: {
    ...
    fields: [
      ...
      ['loc', '当前库位码', KIND.TEXT],
      ['status', '状态', KIND.SELECT],
      ['version', '业务版本', KIND.NUM],
      ['lastOpId', '最后操作ID', KIND.TEXT]
    ]
  },
  members: {
    table: '人员', key: '编号',
  ...
  items: {
    ...
    fields: [
      ...
      ['version', '业务版本', KIND.NUM],
      ['lastOpId', '最后操作ID', KIND.TEXT],
      ['materialCode', '关联物料码', KIND.TEXT]
    ]
  },
  itemOperations: {
    table: '物品操作', key: '操作ID',
    fields: [
      ['code', '操作ID', KIND.TEXT], ['kind', '操作类型', KIND.TEXT],
      ['itemCode', '物品码', KIND.TEXT], ['containerCode', '容器码', KIND.TEXT],
      ['request', '请求内容', KIND.JSONF], ['requestHash', '请求摘要', KIND.TEXT],
      ['before', '操作前快照', KIND.JSONF], ['after', '目标快照', KIND.JSONF],
      ['phase', '处理阶段', KIND.SELECT], ['progress', '执行进度', KIND.JSONF],
      ['operator', '操作人', KIND.TEXT], ['device', '设备', KIND.TEXT],
      ['requestedAt', '受理时间', KIND.ISO], ['finishedAt', '完成时间', KIND.ISO],
      ['error', '错误与恢复说明', KIND.TEXT]
    ]
  },
  manuals: {
    table: '手册', key: '手册码',
...
const GENERIC_WRITE_TABLES = Object.freeze(['materials', 'locations', 'containers', 'members', 'items', 'manuals', 'workorders', 'transactions']);
const READ_TABLES = Object.freeze([...GENERIC_WRITE_TABLES, 'itemOperations']);
const OPERATION_TABLES = Object.freeze(['itemOperations']);
const APPEND_PROTECTED_TABLES = Object.freeze(['transactions', 'itemOperations']);
// Compatibility export only. Read consumers below explicitly use READ_TABLES.
const PUSH_ORDER = GENERIC_WRITE_TABLES;
...
function mapUp(def, obj) {
  ...
  (def.fields || []).forEach(f => {
    ...
    /* 物品化工单的「执行数量」列：execQty 恒为空，已执行的是 execItems 里的物品码。
       UP 只拿到单个字段值，判不了形态，所以工单表在这里按整单形态分流。 */
    const v = (def === TABLE_DEFS.workorders && localKey === 'execQty' && isItemizedOrderShape(obj))
      ? (Array.isArray(obj.execItems) && obj.execItems.length ? obj.execItems.join('; ') : '')
      : (UP[kind] || UP.text)(obj[localKey]);
...
function mapDown(def, f, present) {
  ...
  if (def === TABLE_DEFS.items && (!present || present.has('状态')) && !out.status) out.status = 'unknown';
  /* 物品化工单的「执行数量」列存的是已 APPLIED 物品码：DOWN.execqty 把它们解析成
     {itemCodes:[…]} 行，这里按整单形态拆到 execItems，execQty 只留旧形态行。 */
  if (def === TABLE_DEFS.workorders && isItemizedOrderShape(out) && Array.isArray(out.execQty)) {
    const codes = [];
    out.execQty = out.execQty.filter(e => {
      if (e && Array.isArray(e.itemCodes)) { codes.push(...e.itemCodes); return false; }
      return true;
    });
    if (codes.length) out.execItems = codes;
  }
  return out;
}
...
async function listRecordsEx(token, tableId) {
  ...
  do {
    ...
    const r = await feishuFetch(u, { headers: { Authorization: 'Bearer ' + token } });
    const j = await r.json();
...
async function pullState() {
  ...
  await Promise.all(READ_TABLES.map(async key => {
...
async function listFields(token, tableId) {
  ...
  return (j.data.items || []).map(f => ({
    id: f.field_id,   /* 2.62.0（审计 B-d）：字段 id 与原始选项对象——单选补选项 PUT 需要 */
    ...
    optionsRaw: (f.property && f.property.options) ? JSON.parse(JSON.stringify(f.property.options)) : undefined
  }));
}

/** 2.62.0（审计 B-d）：向单选列追加选项——只新增，不改不删现有选项。
 * 飞书 PUT fields/{id} 要求回传完整 options 列表，旧选项必须带原 id，
 * 否则已有记录的选项值会被断开关联。重名幂等跳过。 */
async function addSelectOptions(token, tableId, fieldName, addNames) {
  const fields = await listFields(token, tableId);
  const f = fields.find(x => x.name === fieldName && Array.isArray(x.optionsRaw));
  if (!f) throw fsError('补齐单选选项', { code: -1, msg: '列「' + fieldName + '」不是单选或不存在' });
  const existing = new Set(f.options || []);
  const fresh = (addNames || []).map(n => String(n || '').trim()).filter(n => n && !existing.has(n));
  if (!fresh.length) return { ok: true, added: [] };
  const options = f.optionsRaw.concat(fresh.map(n => ({ name: n })));
  const u = FEISHU_HOST + '/open-apis/bitable/v1/apps/' + BASE_TOKEN + '/tables/' + tableId + '/fields/' + f.id;
  const r = await feishuFetch(u, { method: 'PUT', headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
    body: JSON.stringify({ field_name: f.name, type: f.type, property: { options } }) });
  const j = await r.json();
  if (j.code !== 0) throw fsError('补齐单选选项 ' + fieldName, j);
...
async function batchUpdate(token, tableId, records) {
  const r = await feishuFetch(FEISHU_HOST + '/open-apis/bitable/v1/apps/' + BASE_TOKEN + '/tables/' + tableId + '/records/batch_update', {
    method: 'POST',
...
async function batchCreate(token, tableId, records) {
  const r = await feishuFetch(FEISHU_HOST + '/open-apis/bitable/v1/apps/' + BASE_TOKEN + '/tables/' + tableId + '/records/batch_create', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
    body: JSON.stringify({ records })
  });
...
async function findExistingKeys(token, tableId, keyField, keys) {
  ...
  for (let i = 0; i < list.length; i += KEY_CHUNK) {
    ...
    do {
      ...
      (d.items || []).forEach(r => { const k = T(r.fields[keyField]); if (k) out.set(k, { record_id: r.record_id, fields: r.fields }); });
      pt = d.has_more ? d.page_token : '';
    } while (pt);
  }
  return out;
...
async function batchDelete(token, tableId, ids) {
  ...
  const r = await feishuFetch(FEISHU_HOST + '/open-apis/bitable/v1/apps/' + BASE_TOKEN + '/tables/' + tableId + '/records/batch_delete', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
...
async function writeStock({ matCode, qty, delta, operator, type, reason, ref, opId, dryRun, ts, device }) {
  ...
  if (lastErr) {
    /* 2.70.0 M5（并发方案）：seq 重试耗尽——删除自己创建的行再报错。
       此时 qty 还没改（qty 更新在 seq 循环之后），这行是幽灵账本行（余量不真实），
       留着会污染期初推算链。清理后用户重试即可，不留残余。 */
    if (createdId) { try { await batchDelete(token, TABLES.transactions, [createdId]); } catch (_) { } }
...
async function syncChanges(watermarks) {
  ...
  await Promise.all(changed.map(async key => {
    try {
      ...
      changes[key] = r.complete ? r.records : [];
      nextWm[key] = r.complete ? r.watermark : (watermarks[key] || { ts: 0, seen: [] });
      if (!r.complete) probe.tables[key].error = 'incremental-pagination-incomplete';
...
async function pullChangesBySort(keyName, watermark, opts) {
  ...
  const raw = [];
  let pageToken = '', pages = 0, complete = false;
  ...
  while (pages < maxPages) {
    ...
    if (items.length && N(items[items.length - 1].fields && items[items.length - 1].fields[LAST_UPDATED_COL]) < wm.ts) { complete = true; break; }
    if (!d.has_more) { complete = true; break; }
    // Even a short page may advertise more data. Never advance on truncated/malformed pagination.
    if (!items.length || !d.page_token || visitedTokens.has(d.page_token)) break;
  ...
  return { records, raw, watermark: complete ? { ts, seen: [...seen] } : wm, pages, complete, columns: [...present] };
}
...
async function maxCodeSuffix(p) {
  ...
  try {
    ...
    const d = await searchRecords(token, tableId, {
      pageSize: 500,
      fieldNames: [column].concat(typeColumn ? [typeColumn] : []),   /* 2.49.5（审计 bug3）：searchRecords 读 opts.fieldNames，snake_case 会被静默丢弃 */
      sort: [{ field_name: column, desc: true }],
      filter
    });
    items = d.items || [];
    /* 2.50.1（审计 bug3）：search 只拿 1 页且按文本列字典序排——同前缀 >500 条混宽数值时
       最大号会沉底（实证 next=1000 而页外 1000~1003 已存在 → 撞号）。翻页未尽就整表读。 */
    if (d.has_more) items = await listRecords(token, tableId);
  } catch (_) {
...
async function upsertRecords(keyName, records, opts) {
  opts = opts || {};
  if (OPERATION_TABLES.includes(keyName)) return { error: '物品操作表只允许专用接口写入' };
  const def = TABLE_DEFS[keyName];
  ...
  const uniqueDomain = require('./unique-items.js');
  const guarded = require('./item-schema.js').isControlledSchema(keyName, defs);
  if (guarded) {
    records = (records || []).map(r => uniqueDomain.ordinaryFields(keyName, r));
    opts = { ...opts, clearFields: (opts.clearFields || []).filter(k => ![...(uniqueDomain.CONTROLLED[keyName] || []), ...(keyName === 'items' ? ['loc', 'qty'] : [])].includes(k)) };
  }
  ...
  (records || []).forEach(r => {
    const upFields = mapUp(def, r);
    if (guarded) {
      const allowed = new Set(Object.keys(uniqueDomain.ordinaryFields(keyName, r)));
      def.fields.forEach(([local, col]) => { if (!allowed.has(local)) delete upFields[col]; });
    }
    const c = coerceFields(defs, upFields, clearCols);
    ...
    if (hit) {
      ...
      /* 2.61.1 Phase S3（脏字段推送）：只把与飞书现有值不同的列放进 batchUpdate——
         多设备并发时，各设备只推自己改过的字段，不再整行覆盖丢对方字段。
         值相同的列跳过（不产生多余写请求）。 */
      const existingFields = hit.fields || {};
      const dirtyFields = {};
      let hasDirty = false;
      Object.keys(fields).forEach(col => {
        const oldVal = existingFields[col];
        const newVal = fields[col];
        if (JSON.stringify(oldVal) !== JSON.stringify(newVal)) { dirtyFields[col] = newVal; hasDirty = true; }
      });
      if (hasDirty) toUpdate.push({ record_id: hit.record_id, fields: dirtyFields });
    }
  ...
  if (guarded && toCreate.length) {
    return { created: 0, updated: 0, error: '受控实体建档须经认证与共享唯一认领；通用 upsert 不接受新码', dropped: droppedAll, droppedColumns };
  }
  if (opts.dryRun) {
    return {
...
async function deleteRecords(keyName, keys) {
  if (OPERATION_TABLES.includes(keyName)) return { deleted: 0, error: '物品操作历史不可删除' };
  const def = TABLE_DEFS[keyName];
  ...
  const token = await tenantToken();
  if (['items', 'containers', 'locations'].includes(keyName)) {
    const defs = await listFields(token, tableId);
    if (require('./item-schema.js').isControlledSchema(keyName, defs)) return { deleted: 0, error: '受控实体不可硬删或回收编码，请停用/退役' };
  }
  const want = [];
  (keys || []).forEach(k => {
  ...
  const ids = want.map(k => { const v = idx.get(k); return v ? (typeof v === 'object' ? v.record_id : v) : null; }).filter(Boolean);
  if (!ids.length) return { deleted: 0, notFound: want };

  const res = await feishuFetch(FEISHU_HOST + '/open-apis/bitable/v1/apps/' + BASE_TOKEN + '/tables/' + tableId + '/records/batch_delete', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
    body: JSON.stringify({ records: ids })
  });
...
async function reconcile(local) {
  ...
  const summary = { missingColumns: 0, missingOptions: 0, localOnly: 0, remoteOnly: 0, contentDiff: 0, duplicateRemoteKeys: 0 };
  ...
  await Promise.all(READ_TABLES.map(async key => {
    ...
    const normVal = v => {
      if (v == null) return '';
      /* itemCodes 必须保留：物品化工单的明细/执行数量就是码列表，
         丢掉它会把「码不同」的物品化单误判成内容一致 */
      if (Array.isArray(v)) return JSON.stringify(v.map(x => (x && typeof x === 'object')
        ? { matCode: x.matCode, qty: x.qty, itemCodes: x.itemCodes } : x));
      if (typeof v === 'object') return JSON.stringify(v);
      if (typeof v === 'number') return String(Math.round(v * 1e6) / 1e6);
      return String(v).trim();
    };
    const remoteByKey = Object.create(null);
    /* v3.13.6（发现 J）：飞书侧两条记录换算后**同业务键**（脏数据）时，
       旧写法 `remoteByKey[k]=r.fields` 会让后者静默覆盖前者 —— 差异根本不报，
       用户以为两边一致。改为记录重复，汇总时如实告知。 */
    const duplicateRemoteKeys = [];
    recs.forEach(r => {
      const k = String(def.fromFeishuKey(T(r.fields[def.key])));
      if (k && remoteByKey[k]) { if (duplicateRemoteKeys.indexOf(k) < 0) duplicateRemoteKeys.push(k); return; }
      remoteByKey[k] = r.fields;
    });
    ...
    ((local && local[key]) || []).forEach(lr => {
      ...
      (def.fields || []).forEach(([localField, col, kind]) => {
        ...
        let lv = lr[localField];
        /* 物品化工单的「执行数量」列：本地已执行码在 execItems（execQty 恒空），
           远端 DOWN 出来是 {itemCodes:[…]} 行 —— 比之前先把本地拼成同一形态，
           否则每张物品化单都会报一条假的 contentDiff。 */
        if (def === TABLE_DEFS.workorders && localField === 'execQty' && isItemizedOrderShape(lr)) {
          lv = (Array.isArray(lr.execQty) ? lr.execQty : [])
            .concat(Array.isArray(lr.execItems) && lr.execItems.length ? [{ itemCodes: lr.execItems }] : []);
        }
    ...
    summary.duplicateRemoteKeys += duplicateRemoteKeys.length;
  }));
...
/* ---------- HTTP 小工具 ---------- */
/* 2.50.2（审计 I-B6 高危）：API 在公网上裸奔（CORS * + 无鉴权）= 任意网页都能读写成员/物品数据。
   两层收敛（不破坏同源页面使用）：
   ① CORS 只对已知来源放行 —— 跨站脚本无法再读响应（数据外泄面收口）；
   ② 写接口额外要求自定义头 X-416mes-Same-Origin: 1 —— 浏览器跨站发自定义头必须过预检，
      服务端不放行 → 跨站写入被浏览器拦截（服务端直连如 curl 不受 CORS 约束，
      那属于部署层鉴权决策：上 Vercel Deployment Protection 或反向代理加 token）。 */
const CORS_ALLOW_ORIGINS = ['https://mes.newenergycoder.club'];
const CORS_ALLOW_ORIGIN_RE = [/^https:\/\/([a-z0-9-]+\.)?newenergycoder\.club$/, /^http:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/, /^http:\/\/192\.168\.\d+\.\d+(:8000)?$/];
function corsOriginAllowed(origin) {
  if (!origin) return false;
  if (CORS_ALLOW_ORIGINS.includes(origin)) return true;
  return CORS_ALLOW_ORIGIN_RE.some(re => re.test(origin));
}
/* 2.65.0 P0-3（k3 并发审计）：飞书 fetch 出口统一包装——
   ① 429/99991400 频控按 1s/2s/4s 退避重试（最多 3 次），仍失败抛「飞书繁忙」；
   ② 进程级每秒令牌桶（~12 QPS），压住 serverless 冷启动并发的瞬时请求。 */
let _fsTokens = 30, _fsTs = Date.now();
async function fsThrottle() {
  for (;;) {
    const now = Date.now();
    _fsTokens = Math.min(30, _fsTokens + (now - _fsTs) / 1000 * 20);
    _fsTs = now;
    if (_fsTokens >= 1) { _fsTokens -= 1; return; }
    await new Promise(r => setTimeout(r, Math.ceil((1 - _fsTokens) / 20 * 1000)));
  }
}
/* 2.81.0 C1（稳定多并发）：每命令飞书调用埋点——AsyncLocalStorage 隔离并发请求
   （serverless 温实例并发处理多请求，模块级计数器会串数）。 */
const { AsyncLocalStorage } = require('node:async_hooks');
const feishuMetrics = new AsyncLocalStorage();
async function feishuFetch(url, opts) {
  for (let attempt = 0; ; attempt++) {
    await fsThrottle();
    const m = feishuMetrics.getStore();
    if (m) { m.calls++; }
    const r = await fetch(url, opts);
    let j = null;
    try { j = await r.clone().json(); } catch (e) { }
    const rateLimited = r.status === 429 || (j && (j.code === 99991400 || j.code === 1254290));
    if (m && rateLimited) { m.rateLimited = (m.rateLimited || 0) + 1; }
    if (!rateLimited || attempt >= 3) return r;
    const wait = 1000 * Math.pow(2, attempt) * (0.85 + Math.random() * 0.3);
...
const SAME_ORIGIN_HEADER = 'x-416mes-same-origin';
function assertSameOrigin(req, res) {
  /* 只拦「带 Origin 的浏览器跨站写」：同源页面（自带标记）与服务器直连（无 Origin，
     如 curl/服务端任务）放行——后者属部署层鉴权范畴（见 I-B6 报告）。 */
  const origin = req && req.headers && req.headers.origin;
  if (!origin) return true;
  if (corsOriginAllowed(origin) && req.headers[SAME_ORIGIN_HEADER] === '1') return true;
  res.status(403).json({ ok: false, error: '缺少同源标记（X-416mes-Same-Origin），本接口不开放跨站调用' });
  return false;
}
...
module.exports = {
  READ_TABLES, GENERIC_WRITE_TABLES, OPERATION_TABLES, APPEND_PROTECTED_TABLES,
  ...
  tenantToken, listRecords, listRecordsEx, listFields, fieldNames, validateFields, pullState, reconcile, maxCodeSuffix,
  ...
  fsError, setCors, assertSameOrigin, readBody, resetTokenCache, addSelectOptions, readRecord, feishuMetrics
};
