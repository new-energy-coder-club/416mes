...
async function writeStock({ matCode, qty, delta, operator, type, reason, ref, opId, dryRun }) {
  ...
  let hit, txnDefs, rowsTmp;
  try {
    const t = Date.now();
    const [hitR, defsR] = await Promise.all([
      findOne(token, TABLES.materials, '物料码', matCode),
      listFieldsCached(token, TABLES.transactions)
    ]);
    hit = hitR; txnDefs = defsR; rowsTmp = 2; bump(); bump(); mark('lookup', t);
  } catch (e) { return { ok: false, error: String((e && e.message) || e) }; }
  ...
  if (opId && hasOpCol) {
    let dup = null;
    try { const t = Date.now(); dup = await findOne(token, TABLES.transactions, '操作ID', String(opId)); bump(); timing.lookup += Date.now() - t; } catch (_) { dup = null; }
    if (dup) {
      const f = dup.fields || {};
      return {
