...
async function searchRecords(token, tableId, opts) {
  opts = opts || {};
  const body = { page_size: Math.min(opts.pageSize || 500, 500) };
  ...
  const r = await fetch(FEISHU_HOST + '/open-apis/bitable/v1/apps/' + BASE_TOKEN + '/tables/' + tableId + '/records/search', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
...
async function dumpSearchShape() {
  ...
  const keyField = TABLE_DEFS.materials.key;
  ...
  const out = { table: '物料台账', keyField, probes: [] };
  out.probes.push(await send('全量 page_size=500', { pageSize: 500 }));
  out.probes.push(await send('page_size=1', { pageSize: 1 }));
  out.probes.push(await send('page_size=1 + field_names=[主键]', { pageSize: 1, fieldNames: [keyField] }));
  out.probes.push(await send('page_size=1 + sort', { pageSize: 1, sort: [{ field_name: keyField, desc: true }] }));
  return out;
}
