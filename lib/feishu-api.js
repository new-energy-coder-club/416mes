      ...
      if (dateField) {
        ...
        try {
          const d4 = await searchRecords(token, tableId, {
            pageSize: 1,
            filter: { conjunction: 'and', conditions: [{ field_name: dateField.name, operator: 'isGreater', value: ['Exclusive', 0] }] }
          });
          rec.filterByDateGreater = 'ok';
          rec.filterByDateTotal = d4.total;
        } catch (e) { rec.filterByDateGreater = String(e.message).slice(0, 220); }
      }
    }
    out.tables[key] = rec;
  }));
...
module.exports = {
  ...
  searchRecords, probeChangeDetection,
