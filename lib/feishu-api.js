...
async function dumpSearchShape() {
  ...
  out.probes.push(await send('field_names=[主键] + page_size=500', { pageSize: 500, fieldNames: [keyField] }));
  out.probes.push(await send('field_names=[主键] + page_size=1', { pageSize: 1, fieldNames: [keyField] }));
  out.probes.push(await send('field_names=[最后更新时间] + page_size=1 + sort desc', { pageSize: 1, fieldNames: ['最后更新时间'], sort: [{ field_name: '最后更新时间', desc: true }] }));
  return out;
}
