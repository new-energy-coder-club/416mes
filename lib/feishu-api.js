...
async function writeStock({ matCode, qty, delta, operator, type, reason, ref, opId, dryRun }) {
  ...
  let token;
  try { token = await tenantToken(); } catch (e) { return { ok: false, error: String((e && e.message) || e) }; }

  /* 1) 找物料行：按业务键精确查一条，不再全表扫（几万条时全表扫是真正的性能悬崖） */
  let hit;
  try {
    hit = await findOne(token, TABLES.materials, '物料码', matCode);
  } catch (e) { return { ok: false, error: String((e && e.message) || e) }; }
  if (!hit) return { ok: false, error: '物料码 ' + matCode + ' 不在飞书台账（请先在网页端建档并 push，或先在飞书表里建这条物料）' };

  /* 2) 幂等：操作ID 已经落库就是重放，直接返回上次结果，绝不再记一次账 */
  let txnDefs;
  try { txnDefs = await listFields(token, TABLES.transactions); }
  catch (e) {
    // 流水表读不到 → 干脆不动库存。否则会出现「库存改了、没有任何凭据」的脱节状态。
    return { ok: false, error: String((e && e.message) || e) };
  }
  const hasOpCol = txnDefs.some(d => d.name === '操作ID');
  if (opId && hasOpCol) {
    let dup = null;
    try { dup = await findOne(token, TABLES.transactions, '操作ID', String(opId)); } catch (_) { dup = null; }
    if (dup) {
      const f = dup.fields || {};
      return {
  ...
  let sumBefore = 0;
  try { sumBefore = await sumDeltas(token, matCode); } catch (_) { sumBefore = 0; }
  ...
  for (let attempt = 0; attempt < SEQ_TRIES; attempt++) {
    ...
    if (attempt === 0) {
      try {
        ...
        const data = await batchCreate(token, TABLES.transactions, [{ fields: c.fields }]);
    ...
    try {
      ...
      let same = await findMany(token, TABLES.transactions, '流水号', '#' + pad(seq, 6));
      if (same.length <= 1) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    try { sumNow = await sumDeltas(token, matCode); } catch (_) { /* 账本读不到就退回本次增量 */ }
    const want = opening + sumNow;
    try {
      await batchUpdate(token, TABLES.materials, [{ record_id: hit.record_id, fields: { '库存数量': want } }]);
    } catch (e) {
    ...
    balance = want;
    const after = await readQty(token, hit.record_id);
    if (after === want) { converged = true; break; }
  }

  return {
    ok: true, seq, opId: opId || undefined, before, balance,
    delta: actualDelta, dropped: txnDropped,
...
async function pullChangesBySort(keyName, watermark, opts) {
  ...
  const records = raw
    .map(it => Object.assign({ __rid: it.record_id, __ts: N(it.fields && it.fields[LAST_UPDATED_COL]) },
      { data: mapDown(def, it.fields, present) }))
    .filter(x => x.data && (keyName === 'transactions' ? true : x.data.code))
    .map(x => Object.assign({}, x.data, { __rid: x.__rid, __ts: x.__ts }));
...
module.exports = {
  ...
  LAST_UPDATED_COL, probeTableChange, probeAllChanges, pullChangesBySort, censusTable,
