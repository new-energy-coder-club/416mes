...
async function writeStock({ matCode, qty, delta, operator, type, reason, ref, opId, dryRun }) {
  ...
  let hit;
  try {
    const t = Date.now(); hit = await findOne(token, TABLES.materials, '物料码', matCode); bump(); mark('lookup', t);
  } catch (e) { return { ok: false, error: String((e && e.message) || e) }; }
  if (!hit) return { ok: false, error: '物料码 ' + matCode + ' 不在飞书台账（请先在网页端建档并 push，或先在飞书表里建这条物料）' };

  /* 2) 幂等：操作ID 已经落库就是重放，直接返回上次结果，绝不再记一次账 */
  let txnDefs;
  try { const t = Date.now(); txnDefs = await listFields(token, TABLES.transactions); bump(); timing.lookup += Date.now() - t; }
  catch (e) {
