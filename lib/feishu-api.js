 ...
 * 返回 { fields, dropped }：dropped 里说明每个被跳过的列及原因，便于排查。
 */
function toMs(v) {
...
function coerceFields(defs, fields) {
  ...
  Object.entries(fields || {}).forEach(([k, v]) => {
    const d = byName[k];
    if (!d) { dropped.push(k); return; }                       // 表里没这列
    ...
    switch (d.typeName) {
      ...
      case '单选':
        ...
        if (d.options && d.options.length && !d.options.includes(String(v))) {
          dropped.push(k + '（单选无此选项：' + v + '，现有 ' + d.options.join('/') + '）');
        } else out[k] = String(v);
        break;
      case '数字':
        if (!empty && isFinite(Number(v))) out[k] = Number(v);
        else if (!empty) dropped.push(k + '（数字非法：' + v + '）');
        break;
      case '日期':
        if (empty) break;
        { const t = toMs(v); if (t != null) out[k] = t; else dropped.push(k + '（日期非法：' + v + '）'); }
        break;
      case '复选框':
        if (!empty) out[k] = !!v;
        break;
      default:
        ...
        if (!empty) out[k] = v;
        else dropped.push(k + '（' + d.typeName + '类型不接受空值，已跳过）');
    }
  });
...
async function upsertRecords(keyName, records) {
  ...
  (records || []).forEach(r => {
    const c = coerceFields(defs, def.up(r));
    c.dropped.forEach(d => dropped.add(d));
...
module.exports = {
  ...
  batchUpdate, batchCreate, coerceFields, toMs, upsertRecords, deleteRecords, writeStock,
