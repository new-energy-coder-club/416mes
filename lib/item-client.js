...
function create({persistence,fetch:fetcher,timeoutMs=45000,pollTries=20,pollGapMs=6000}){   // 试运行模式走飞书多次串行写，实测常超15s；超时仅表示结果未知
 ...
 /* v3.13.1（BUG-11/12）：device 也走 header。
    原先是在 item-persistence.enqueue() 里往 request 补 device，导致
    「落库那份有、提交/回执那份没有」→ canonical 校验失败 → RESULT_REQUEST_MISMATCH
    → 工单执行成功却不记账。走 header 不碰 request 主体，从根上避免。 */
 const deviceHeader=()=>{try{
   const st=(typeof state!=='undefined')?state:null;
   const d=(st&&st.deviceId)||'';
   const safe=String(d).replace(/[\r\n\t]/g,'').trim();
   return /^[A-Za-z0-9 _\-.]+$/.test(safe)?encodeURIComponent(safe):'';
 }catch(e){return '';}};
 const headers=()=>({'Content-Type':'application/json','X-416mes-Same-Origin':'1','X-416mes-Token':(function(){try{return localStorage.getItem('mes416_itm_token')||'';}catch(e){return '';}})(),'X-416mes-Operator':operatorHeader(),'X-416mes-Device':deviceHeader()});
