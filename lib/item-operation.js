  ...
  async function post(req, request) {
    ...
    try {
      ...
      if (mode === 'feishu-trial') {
        ...
        let conflictsVisible = visible.some(o => {
        ...
        /* 2.65.0 P0-2（k3 S2 实证对称互杀）：同实体 PREPARED 并发先等-重查（最多 3 轮，总 ~5s），
           让先到者收尾后再重计划——把双杀变成后到者等待放行或确定性拒绝。 */
        if (conflictsVisible) {
          for (let waitRound = 0; waitRound < 3 && conflictsVisible; waitRound++) {
            await new Promise(r2 => setTimeout(r2, 1200 + Math.floor(Math.random() * 800)));
            const vis2 = await repository.allOperations();
            conflictsVisible = vis2.some(o => {
              if (o.code === frozen.opId || !['PREPARED', 'REPAIR_REQUIRED'].includes(o.phase)) return false;
              if (o.phase === 'REPAIR_REQUIRED') return true;
              const other = U.entityKeysOf(o.request || {});
              if (!other.size || !myKeys.size) return true;
          ...
          /* 2.68.0（P0-2 修正）：等待后仍冲突——必须显式拒绝，不能静默放行 */
