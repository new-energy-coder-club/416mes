...
function create({ repository, coordinator, authenticate, enabled = false, mode = 'strict', feishuMetrics = null }) {
  ...
  /* v3.13.6（发现 F）：REPAIR_REQUIRED 原先只说「检出部分写入，需人工核对」——
     用户不知道**到底哪几件已经生效**，只能逐件去飞书表核对。
     这里按 before/after 快照算出逐行差异，附在回执里，人工核对从「全表翻」变成「照着清单看」。 */
  const ENTITY_LABEL = { items: '物品', containers: '容器', locations: '库位', materials: '物料', members: '成员', workorders: '工单' };
  function entityKeyOf(table, row) {
    return String((row && (row.code || row.itemCode || row.matCode || row.entityCode)) || '');
  }
  function partialWriteDetail(before, after) {
    const detail = [];
    ['items', 'containers', 'locations', 'materials'].forEach(function (table) {
      const b = (before && before[table]) || [], a = (after && after[table]) || [];
      const bKeys = new Set(b.map(function (r) { return entityKeyOf(table, r); }));
      const aKeys = new Set(a.map(function (r) { return entityKeyOf(table, r); }));
      a.forEach(function (r) {
        const k = entityKeyOf(table, r);
        if (!bKeys.has(k)) detail.push('新增 ' + (ENTITY_LABEL[table] || table) + ' ' + k);
        else {
          const prev = b.find(function (x) { return entityKeyOf(table, x) === k; });
          if (prev && canonical(prev) !== canonical(r)) detail.push('变更 ' + (ENTITY_LABEL[table] || table) + ' ' + k);
        }
      });
      b.forEach(function (r) {
        const k = entityKeyOf(table, r);
        if (!aKeys.has(k)) detail.push('移除 ' + (ENTITY_LABEL[table] || table) + ' ' + k);
      });
    });
    return detail;
  }
  function withPartialDetail(verdict, before, after) {
    const detail = partialWriteDetail(before, after);
    if (!detail.length) return verdict;
    return Object.assign({}, verdict, {
      error: verdict.error + '｜受影响：' + detail.slice(0, 20).join('；') + (detail.length > 20 ? '；等共 ' + detail.length + ' 处' : '')
    });
  }
  ...
  async function sweepStalePrepared() {
    ...
    for (const row of stale) {
      try {
        ...
        else verdict = withPartialDetail({ phase: 'REPAIR_REQUIRED', error: '过期未决命令自动收口：检出部分写入，需人工核对' }, operation.before, operation.after);
  ...
  /* v3.13.6（发现 F）：供 catch 里给 REPAIR_REQUIRED 附「受影响件清单」。
     plan 在 try 内 const 声明，catch 里取不到；这里提前占位。 */
  let planForDetail = null;
    ...
    try {
      ...
      if (sideEffectsPossible) {
        if (mode === 'feishu-trial') {
          ...
          try {
            ...
            if (rows.length === 1) {
              ...
              const finished = verdict || withPartialDetail({ ...cur, phase: 'REPAIR_REQUIRED', error: String(e.message) }, cur.before, cur.after);
        ...
        return withPartialDetail({ code: frozen.opId, request: frozen, phase: 'REPAIR_REQUIRED', error: String(e.message) }, planForDetail && planForDetail.before, planForDetail && planForDetail.after);
      }
  ...
  return { post, get, recover, settle, mode, partialWriteDetail, withPartialDetail };
}
