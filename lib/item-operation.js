  ...
  async function _post(req, request) {
    ...
    try {
      ...
      if (mode === 'feishu-trial') {
        ...
        /* P2b：与 trial 协调器同一套陈旧 PREPARED 过期规则——claim 放行后，
           写前裁决不得再被已过期的僵尸行绊倒（serverless 60s 上限，10 分钟前不可能在途）。 */
        ...
        const conflictsWith = list => list.some(o => {
          ...
          const _ts = Date.parse(o.requestedAt || '');
          if (Number.isFinite(_ts) && Date.now() - _ts > STALE_PREPARED_MS) return false;
        ...
        if (conflictsVisible) {
          ...
          /* P2a（并发模型定稿，用户 2026-09-23 拍板）：「谁先确认谁赢，后到者作废」——
             loser 立即拒绝，不再 20s 预算内等待 winner（等待期客户端只见请求悬着，单设备场景
             还会被误读成卡死）。确定性定序 (requestedAt, opId) 保证 winner 唯一；
             loser 的 PREPARED 行由 loser 自己的请求按拒绝收场。 */
          if (iAmLoser) preWriteError = 'TRIAL_CONCURRENT_OPERATION_DETECTED';
...
function retryPolicyFor(errText) {
  /* P2a（并发模型定稿，2026-09-23）：「先确认者赢、后到者作废」——
     版本类拒绝必须换新 opId 重建（客户端单轮重建，无自动退避）；
     屏障类拒绝在人工/自动收口后**同一 opId 可直接重提**（屏障拒绝发生在 claim 阶段，未留日志、编号未烧掉）。 */
