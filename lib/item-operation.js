  ...
  async function post(req, request) {
    ...
    try {
      ...
      if (mode === 'feishu-trial') {
        ...
        if (visible.some(o => o.code !== frozen.opId && ['PREPARED', 'REPAIR_REQUIRED'].includes(o.phase))) preWriteError = 'TRIAL_CONCURRENT_OPERATION_DETECTED';
        if (!preWriteError) {
          try {
        ...
        if (preWriteError) {
          ...
          await repository.finish(recordId, rejected);
          await coordinator.finish(frozen.opId, rejected);
          return rejected;
        }
      }
