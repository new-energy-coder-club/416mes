  ...
  async function post(req, request) {
    ...
    try {
      ...
      if (mode === 'feishu-trial') {
        ...
        if (visible.some(o => o.code !== frozen.opId && ['PREPARED', 'REPAIR_REQUIRED'].includes(o.phase))) fail('TRIAL_CONCURRENT_OPERATION_DETECTED');
        const currentPlan = U.plan(await repository.snapshot(frozen), frozen, actor);
        if (canonical(currentPlan.before) !== canonical(plan.before) || canonical(currentPlan.after) !== canonical(plan.after)) fail('TRIAL_PRECONDITION_CHANGED');
      }
      ...
      if (sideEffectsPossible) {
        if (mode === 'feishu-trial') {
          try { const rows = await repository.operations(frozen.opId); if (rows.length === 1) await repository.finish(rows[0].recordId, { ...rows[0], phase: 'REPAIR_REQUIRED', finishedAt: new Date().toISOString(), error: String(e.message) }); } catch (_) { /* Never retry entity writes on uncertain transport. */ }
        }
