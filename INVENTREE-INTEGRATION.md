...
本文档不保存管理员密码或 API Token。请在本机通过环境变量或未纳入版本控制的
`inventree-sync.config.json` 配置连接信息：

- `INVENTREE_BASE_URL`：InvenTree 地址，例如 `http://localhost:8001`
- `INVENTREE_TOKEN`：API Token，仅存于本机环境变量或私有配置文件
- 管理员账号密码请通过 InvenTree 本机管理流程设置，不要写入项目文档

配置文件已由 `.gitignore` 排除，提交前请确认 `git status` 不包含任何凭据文件。
